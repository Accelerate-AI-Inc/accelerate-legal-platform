 ...
 * Test user: e2e@accelerate.local / E2eTestPass1!
 */
...
test("rename chat: sidebar rename interaction updates the title", async ({ page }) => {
    ...
    // ("bg-surface-sunk"); inactive items carry APP_SURFACE_HOVER_CLASS
    // ("hover:bg-surface-sunk", a different token), so matching
    // "bg-surface-sunk" distinguishes the active item. (The olp liquid-
    // surface refresh renamed the old "bg-surface-sunk/60" active token.)
    const activeItem = page
        .locator('div.group.relative[class*="bg-surface-sunk"]')
        .first();
...
test("delete chat: sidebar delete action removes the chat from history", async ({ page }) => {
    ...
    const firstRow = page.locator("div.group.relative.h-8.rounded-sm").first();
    ...
    const targetRow = page
        .locator("div.group.relative.h-8.rounded-sm")
