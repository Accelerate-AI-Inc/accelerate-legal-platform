...
test.describe("unauthenticated", () => {
    ...
    test("login with invalid credentials shows error message", async ({
        ...
        await page.fill("#email", "e2e@accelerate.local");
        ...
        /* The login page conditionally renders:
               <div className="text-critical text-sm bg-critical-wash p-3 rounded">
                   {error}
               </div>
           when the `error` state is non-null after a failed signInWithPassword.
           REGRESSION: fails if the error <div className="bg-critical-wash"> is removed
           from the login form or if the catch block stops setting `error`. */
        await expect(page.locator("div.bg-critical-wash.text-critical")).toBeVisible({
            timeout: 10_000,
        });
    });
    ...
    test("login with valid credentials redirects to /assistant", async ({
        ...
        const email = process.env.E2E_EMAIL ?? "e2e@accelerate.local";
...
/* The logout flow calls supabase.auth.signOut(), which defaults to GLOBAL
   ...
   shared `e2e@accelerate.local` user it would 401 every other parallel worker
...
test.describe("logout (isolated user)", () => {
    ...
    const logoutEmail =
        process.env.E2E_LOGOUT_EMAIL ?? "e2e-logout@accelerate.local";
