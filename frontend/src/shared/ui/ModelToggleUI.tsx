...
const itemClassName =
  "theme-dropdown-item flex cursor-pointer select-none items-center gap-2 rounded-sm px-2.5 py-1.5 text-xs text-ink outline-none transition-colors focus:text-ink data-[highlighted]:text-ink data-[disabled]:pointer-events-none data-[disabled]:opacity-50 [&>*]:pointer-events-none";
  ...
  if (!loading && models.length === 0) {
    return (
      <button
        ...
        className={
          modalInput
            ? `flex h-10 w-full items-center rounded-sm px-3 text-sm text-ink-faint ${SURFACE_INSET_CLASS} ${SURFACE_HOVER_CLASS} transition-colors enabled:cursor-pointer enabled:hover:text-ink disabled:cursor-default focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/40 focus-visible:ring-offset-2`
            : "flex h-8 shrink-0 items-center rounded-sm px-2 text-sm text-ink-faint transition-colors enabled:cursor-pointer enabled:hover:text-ink disabled:cursor-default focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/40 focus-visible:ring-offset-2"
        }
      >
    ...
    <Dropdown open={open} onOpenChange={handleOpenChange}>
      <DropdownTrigger asChild>
        <button
          ...
          className={
            modalInput
              ? `flex h-10 w-full items-center justify-between gap-2 rounded-sm px-3 text-sm text-ink ${SURFACE_INSET_CLASS} ${SURFACE_HOVER_CLASS} transition-colors enabled:cursor-pointer disabled:cursor-default disabled:opacity-60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/40 focus-visible:ring-offset-2 ${open ? SURFACE_SELECTED_CLASS : ""}`
              : `flex h-8 shrink-0 items-center rounded-sm text-sm text-ink-faint transition-colors enabled:cursor-pointer enabled:hover:text-ink disabled:cursor-default disabled:hover:text-ink-faint focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/40 focus-visible:ring-offset-2 ${compact ? "w-8 justify-center px-0" : "gap-1.5 px-2"} ${open ? "text-ink" : ""}`
          }
        >
          {compact ? (
            loading ? (
      ...
      <DropdownContent
        ...
        className={`max-h-[min(420px,70vh)] overflow-y-auto rounded-sm text-ink ${modalInput ? "w-[var(--radix-dropdown-menu-trigger-width)]" : "w-56"}`}
      >
          ...
          return (
            <React.Fragment key={group}>
              ...
              >
                ...
                <ChevronDown
                  className={`h-3.5 w-3.5 text-ink-faint transition-transform duration-200 ${expanded ? "rotate-180" : ""}`}
                />
              </DropdownItem>
              {expanded &&
                items.map((model) => (
                  <DropdownItem
                    ...
                    className={`${itemClassName} ${model.id === value ? "text-ink" : ""}`}
                  ...
                  >
                    ...
                    {model.id === value && (
                      <Check className="ml-1 h-3.5 w-3.5 text-ink-muted" />
                    )}
                  </DropdownItem>
                ))}
            </React.Fragment>
        ...
        {reasoningLevel !== undefined && onReasoningChange && (
          <>
            ...
            <DropdownItem
              ...
              className="block cursor-default rounded-sm px-2.5 py-2 [&>*]:pointer-events-auto"
            >
              <div>
                <div className="mb-1.5 flex items-center gap-2 text-xs text-ink">
                  <span className="font-medium">Reasoning</span>
                  <span className="ml-auto text-[10px] text-ink-muted">
                    {REASONING_LEVEL_LABELS[reasoningLevel]}
                  </span>
                </div>
                <div className="relative h-4 w-full rounded-full bg-surface-sunk/80 transition-shadow duration-150 has-[input:focus-visible]:ring-2 has-[input:focus-visible]:ring-accent/40 has-[input:focus-visible]:ring-offset-2">
                  <span aria-hidden="true" className="absolute inset-0">
                    <span
                      className="absolute inset-y-0 left-0 rounded-full bg-gradient-to-r from-accent/60 via-accent to-accent transition-[width] duration-150 ease-out"
                      style={{
                        width: `calc(${reasoningProgress}% + ${1 - reasoningProgress / 100}rem)`,
                      }}
                    />
                    <span
                      className={`absolute top-1/2 h-3 w-3 rounded-full bg-surface transition-[left,transform] duration-150 ease-out ${SURFACE_OVERLAY_CLASS}`}
                      style={{
