...
describe("same-origin API gateway", () => {
    ...
    it("forwards cookies, origin, query parameters, and encoded path segments", async () => {
        ...
        upstreamHeaders.append(
            "set-cookie",
            "__Host-accelerate-session=one; Path=/; Secure; HttpOnly",
        );
        upstreamHeaders.append(
            "set-cookie",
            "__Host-accelerate-session.1=two; Path=/; Secure; HttpOnly",
        );
        ...
        const request = new NextRequest(
            ...
            {
                headers: {
                    cookie: "__Host-accelerate-session=incoming",
        ...
        expect(forwardedHeaders.get("cookie")).toBe(
            "__Host-accelerate-session=incoming",
        );
        ...
        expect(response.headers.get("set-cookie")).toContain(
            "__Host-accelerate-session=one",
        );
        expect(response.headers.get("set-cookie")).toContain(
            "__Host-accelerate-session.1=two",
        );
    });
