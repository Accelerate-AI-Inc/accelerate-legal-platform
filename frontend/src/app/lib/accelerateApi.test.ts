...
describe("AccelerateApiError / isMfaRequiredError", () => {
    it("carries status and code, defaulting code to null", () => {
        const withCode = new AccelerateApiError({
            message: "nope",
            status: 403,
            code: "mfa_verification_required",
        });
        expect(withCode.name).toBe("AccelerateApiError");
        expect(withCode.status).toBe(403);
        expect(withCode.code).toBe("mfa_verification_required");

        const withoutCode = new AccelerateApiError({ message: "nope", status: 500 });
    ...
    it("recognizes exactly the 403 + mfa_verification_required combination", () => {
        ...
        expect(
            isMfaRequiredError(
                new AccelerateApiError({ message: "x", status: 403, code: "other" }),
            ),
        ).toBe(false);
        expect(
            isMfaRequiredError(
...
describe("apiRequest plumbing (via thin wrappers)", () => {
    ...
    it("maps a JSON error body to a AccelerateApiError with code and detail", async () => {
        fetchMock.mockResolvedValue(
            jsonResponse(
        ...
        expect(error).toBeInstanceOf(AccelerateApiError);
        const apiError = error as AccelerateApiError;
...
describe("blob requests (exportAccountData)", () => {
    ...
    it("throws a AccelerateApiError on failure", async () => {
        fetchMock.mockResolvedValue(
...
// Error carrying the raw response text instead of AccelerateApiError.
// ---------------------------------------------------------------------------
...
describe("multipart upload endpoints", () => {
    ...
    it("uploads workflow reference files as authenticated multipart data", async () => {
        ...
        ).rejects.toBeInstanceOf(AccelerateApiError);
    });
