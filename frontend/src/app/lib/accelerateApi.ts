/**
 * Accelerate Legal API client — all browser requests use the same-origin `/api` gateway.
...
export function isMfaRequiredError(error: unknown) {
    return (
        error instanceof AccelerateApiError &&
        error.status === 403 &&
...
async function toApiError(response: Response, path: string) {
    ...
    try {
        ...
        devLog("[accelerate-api] non-ok response", {
            path,
            status: response.status,
            code: parsed.code,
            requestId,
        });
        return new AccelerateApiError({
            status: response.status,
        ...
        devLog("[accelerate-api] non-ok non-json response", {
            path,
            status: response.status,
...
export interface LegalResourceAttribution {
    source: string;
    license: string;
}
...
export interface LegalResourcePage {
    ...
    limit: number;
    offset: number;
    /** True when the backend served the bundled dataset, not the seeded table. */
    bundled: boolean;
    attribution: LegalResourceAttribution;
}

export interface LegalResourceFilterOptions {
    kinds: string[];
    categories: string[];
    languages: string[];
    jurisdictions: string[];
...
export async function listLegalResources(
    ...
    return apiRequest<LegalResourcePage>(
        `/legal-resources${search ? `?${search}` : ""}`,
    );
}

export async function getLegalResourceFilterOptions(): Promise<LegalResourceFilterOptions> {
    return apiRequest<LegalResourceFilterOptions>(
        "/legal-resources/filter-options",
    );
}

export async function getLegalResource(
    slug: string,
): Promise<LegalResourceSummary & { attribution: LegalResourceAttribution }> {
    return apiRequest<
        LegalResourceSummary & { attribution: LegalResourceAttribution }
    >(`/legal-resources/${encodeURIComponent(slug)}`);
}
