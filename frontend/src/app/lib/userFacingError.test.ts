...
import { AccelerateApiError } from "./accelerateApi";
import {
    errorCode,
...
describe("userFacingApiError", () => {
    it("allows intentional client-error details", () => {
        const error = new AccelerateApiError({
            status: 400,
    ...
    it("rejects non-client statuses and empty client-error messages", () => {
        expect(
            userFacingApiError(
                new AccelerateApiError({ status: 399, message: "Unexpected" }),
                "Fallback",
            ),
        ).toBe("Fallback");
        expect(
            userFacingApiError(
                new AccelerateApiError({ status: 400, message: "" }),
                "Fallback",
            ),
        ).toBe("Fallback");
    });
