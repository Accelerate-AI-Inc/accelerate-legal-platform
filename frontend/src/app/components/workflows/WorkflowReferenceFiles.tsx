...
>(function WorkflowReferenceFiles(
  ...
  return (
    <>
      ...
      />
      {error && (
        <p className="mx-4 mb-2 -mt-1 text-xs text-critical md:mx-8">{error}</p>
      )}
      <TableScrollArea
        header={
          <TableHeaderRow>
          ...
          <TableBody>
            {files.map((file) => (
              <TableRow key={file.id} interactive={false}>
                ...
                <TableCell className="ml-auto w-20 text-xs uppercase text-ink-muted">
                  {file.file_type || "—"}
                </TableCell>
                <TableCell className="w-24 text-xs text-ink-muted">
                  {formatBytes(file.size_bytes)}
                </TableCell>
                <TableCell className="w-32 text-xs text-ink-muted">
