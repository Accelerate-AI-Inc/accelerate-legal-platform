...
>["components"] = {
  h1: ({ children }) => (
    <h3 className="mb-1 mt-4 text-base font-semibold leading-tight text-ink first:mt-0">
      {children}
    </h3>
  ),
  h2: ({ children }) => (
    <h3 className="mb-1 mt-3 text-sm font-semibold leading-tight text-ink first:mt-0">
      {children}
    </h3>
  ),
  h3: ({ children }) => (
    <h3 className="mb-0.5 mt-2 text-xs font-semibold leading-tight text-ink first:mt-0">
      {children}
    </h3>
  ),
  ...
  table: ({ children }) => (
    <div className="my-3 overflow-x-auto rounded-sm border border-rule-strong/70 first:mt-0 last:mb-0">
  ...
  th: ({ children }) => (
    <th className="border-b border-r border-rule-strong/70 px-3 py-2 font-medium last:border-r-0">
      {children}
    </th>
  ),
  td: ({ children }) => (
    <td className="border-b border-r border-rule-strong/70 px-3 py-2 align-top text-ink last:border-r-0">
      {children}
    </td>
  ),
};
...
function SkillViewer({ skill }: { skill: string }) {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden">
      <div className="flex min-h-10 shrink-0 items-center justify-between border-b border-rule/60 px-3 py-1.5">
        <span className="text-xs font-medium text-ink-muted">SKILL.md</span>
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className={`flex h-7 w-7 items-center justify-center rounded-sm text-ink-muted transition-colors hover:bg-surface/60 hover:text-ink ${
              showRaw ? "bg-surface/70 text-ink" : ""
            }`}
          >
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-5">
        {showRaw ? (
          <pre className="whitespace-pre-wrap font-mono text-xs leading-5 text-ink">
            {skill.trim()}
          </pre>
        ) : (
          <div
            ref={previewRef}
            className="space-y-3 font-serif text-sm leading-relaxed text-ink-muted"
          >
            <ReactMarkdown
    ...
    >
      {tabs.map((tab) => (
        <button
          ...
          className={`inline-flex h-7 items-center justify-center rounded-full px-3 text-xs font-medium ${SURFACE_INSET_CLASS} transition-all ${
            active === tab.id
...
function MetadataItem({ label, value }: { label: string; value: string }) {
  return (
    <div className="min-w-0">
      ...
      <div className="mt-0.5 max-w-56 truncate text-xs font-medium text-ink">
        {value || "—"}
      </div>
    </div>
  );
    ...
    >
      <div className="flex min-h-0 flex-1 flex-col pb-3">
        <div className="flex shrink-0 items-start justify-between gap-4 pb-5">
          <h2 className="min-w-0 font-serif text-xl font-medium leading-tight tracking-tight text-ink">
            {addon.title}
          </h2>
        </div>
        ...
        <div className="mt-4 mb-2 flex min-h-0 flex-1 flex-col">
          ...
          <div
            className={`${INSET_PANEL_SURFACE_CLASS} flex min-h-0 flex-1 flex-col overflow-hidden rounded-sm`}
          >
            {view === "columns" ? (
              <>
                <div className="grid min-h-10 shrink-0 grid-cols-[52px_0.8fr_1.8fr] items-center border-b border-rule/60 text-xs font-medium text-ink-muted">
                  <div className="px-3 py-1.5">#</div>
                  <div className="border-l border-rule/60 px-3 py-1.5">
                    Column
                  </div>
                  <div className="border-l border-rule/60 px-3 py-1.5">
                    Prompt
                ...
                <div className="min-h-0 flex-1 overflow-y-auto">
                  {(addon.columns_config ?? []).map((column) => (
                    <div
                      ...
                      className="grid grid-cols-[52px_0.8fr_1.8fr] border-b border-rule/50 text-xs last:border-b-0"
                    >
                      <div className="px-3 py-3 text-ink-faint">
                        {column.index + 1}
                      </div>
                      <div className="border-l border-rule/50 px-3 py-3 font-medium text-ink">
                        {column.name}
                      </div>
                      <div className="border-l border-rule/50 px-3 py-3 text-ink-muted">
                        {column.prompt}
                      </div>
                    </div>
                  ))}
              ...
              <div className="flex h-full min-h-0 flex-col">
                <div className="flex min-h-10 shrink-0 items-center border-b border-rule/60 px-3 py-1.5 text-xs font-medium text-ink-muted">
                  Assets
                </div>
                <div className="min-h-0 flex-1 overflow-y-auto">
                  {references.map((file) => (
                    <div
                      key={file.id}
                      className="flex items-center gap-3 border-b border-rule/50 px-3 py-3 text-xs last:border-b-0"
                    >
                      <FileTypeIcon
