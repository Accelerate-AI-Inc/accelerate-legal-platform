...
function AppToolbarButton({
  ...
  return (
    <Button
      ...
      aria-label={title}
      aria-pressed={active}
      className={`h-7 w-7 text-ink-muted hover:bg-surface hover:text-ink ${
        active ? "bg-rule-strong text-ink hover:bg-rule-strong" : ""
      }`}
      onMouseDown={(e) => {
...
export function WorkflowPromptEditor({
  ...
  return (
    ...
    >
      {!readOnly && editor && (
        <div className="flex shrink-0 items-center gap-0.5 border-b border-rule/70 bg-surface px-2 py-1.5">
          <AppToolbarButton
            onClick={() =>
              rawMode
                ? applyRawHeading(1)
          ...
          <div className="w-px h-4 bg-surface-sunk mx-1 shrink-0" />
          <AppToolbarButton
            onClick={() =>
              rawMode
                ? applyRawInline("**")
          ...
          <div ref={tablePickerRef} className="relative">
            ...
            {tablePickerOpen && (
              <div
                ...
                className={`absolute left-0 top-full z-[250] mt-1 w-max rounded-sm p-2 ${SURFACE_OVERLAY_CLASS}`}
              >
                <div
                ...
                <div className="mt-2 text-center text-[11px] font-medium text-ink-muted">
                  {tablePickerSize
      ...
      {readOnly && (
        <div className="flex h-9 shrink-0 items-center justify-between bg-surface px-5">
      ...
      >
        {rawMode ? (
          <textarea
            ...
            readOnly={readOnly}
            spellCheck={false}
            className="h-full min-h-full w-full resize-none bg-transparent px-5 py-4 font-mono text-xs leading-6 text-ink outline-none placeholder:text-ink-faint read-only:cursor-default"
