...
import { MenuSurface } from "@/app/components/ui/menu-surface";
import {
  AssistantIcon,
  ReviewsIcon,
  WorkflowsIcon,
  ...
  const workflowToolbarActions =
    activeTab !== "addons" && selectedWorkflowIds.length > 0 ? (
      <div ref={workflowActionsRef} className="relative">
        ...
        {workflowActionsOpen && (
          <MenuSurface className="absolute top-full right-0 z-[100] mt-1 w-36 overflow-hidden">
            <button
              type="button"
              onClick={() =>
              ...
              className="w-full px-3 py-1.5 text-left text-xs text-critical transition-colors hover:bg-critical/10"
            >
              Delete
            </button>
          </MenuSurface>
    ...
    <>
      ...
      {selectedAddonIds.length > 0 && (
        <button
          ...
          className="inline-flex items-center gap-1 px-1.5 py-1 text-xs font-medium text-ink transition-colors hover:text-ink disabled:opacity-50"
        >
  ...
  return (
    <div className="flex h-full min-h-0 flex-1 flex-col overflow-hidden">
      ...
      {actionError && (
        <div
          role="alert"
          className="flex items-center justify-between gap-3 border-b border-critical bg-critical-wash px-6 py-2 text-sm text-critical"
        >
          <span>{actionError}</span>
          <button
            type="button"
            onClick={() => setActionError("")}
            className="shrink-0 text-xs font-medium text-critical hover:text-critical"
          >
            Dismiss
          </button>
        </div>
          ...
          {[1, 2, 3].map((index) => (
            <TableRow key={index} interactive={false}>
              ...
              <div className="flex w-8 shrink-0 justify-end">
                <div className="h-6 w-6 rounded bg-surface-sunk animate-pulse" />
              </div>
            </TableRow>
          ))}
        </TableBody>
          ...
          <TableBody>
            ...
            return (
              ...
              >
                ...
                <TableCell className="w-52 text-xs text-ink-muted">
                  {workflow.metadata.practice || "—"}
                </TableCell>
                <TableCell className="w-40 truncate text-xs text-ink-muted">
                  {workflow.metadata.jurisdictions?.join(", ") || "—"}
                </TableCell>
                <TableCell className="w-28 text-xs text-ink-muted">
...
function AddonTable({
  ...
  function renderAddonRow(addon: WorkflowAddon, nested = false) {
    const Icon =
      addon.type === "tabular" ? ReviewsIcon : AssistantIcon;
    return (
      <TableRow
        key={addon.id}
      ...
      >
        ...
        <TableCell className="w-20">
          <button
            ...
            className="inline-flex items-center gap-1 px-1 py-1 text-xs font-medium text-ink-muted transition-colors hover:text-ink disabled:opacity-50"
          >
            <Plus className="h-3.5 w-3.5" />
          ...
          {[1, 2, 3].map((index) => (
            <TableRow key={index} interactive={false}>
              ...
              <TableCell className="ml-auto flex w-28 items-center">
                <div className="mr-1.5 h-4 w-4 shrink-0 rounded bg-surface-sunk animate-pulse" />
              ...
              {packs.map((pack) => {
                ...
                return [
                  ...
                  >
                    ...
                    />
                    <TableCell className="ml-auto w-28 text-xs text-ink-muted">
                      Pack
                    </TableCell>
                    <TableCell className="w-52 text-xs text-ink-muted">
                    ...
                    <TableCell className="w-40 text-xs text-ink-muted">
                      —
                    </TableCell>
                    <TableCell className="w-28 text-xs text-ink-muted">
                      —
