  ...
  if (notFound || !workflow) {
    return (
      <div className="flex-1 flex items-center justify-center">
        <p className="text-ink-faint font-serif">Workflow not found.</p>
      </div>
    );
  }
            ...
            ? [
                {
                  type: "custom",
                  render: (
                    <span className="inline-flex h-7 items-center gap-1.5 rounded-full px-3 text-sm text-ink-muted">
                      {saveStatus === "saved" ? (
      ...
      <div className="flex-1 min-h-0 flex flex-col">
        {workflow.metadata.type === "assistant" ? (
          ...
          <>
                      ...
                      selectedColIndices.length > 0 && (
                        <>
                          ...
                          >
                            ...
                            {colActionsOpen && (
                              <div className={`absolute right-0 top-full z-50 mt-1 w-36 overflow-hidden rounded-sm ${SURFACE_OVERLAY_CLASS}`}>
                                <button
                                  onClick={handleDeleteSelectedColumns}
                                  className="w-full px-3 py-1.5 text-left text-xs text-critical hover:bg-critical-wash transition-colors"
                                >
                                  Delete
                                </button>
                              </div>
                          ...
                          <TabPillButton
                            onClick={handleDeleteSelectedColumns}
                            className="text-critical md:hidden"
                          >
                            Delete
                          </TabPillButton>
                        </>
            ...
            {readOnly && (
              <div className="flex h-10 shrink-0 items-center bg-paper px-4 md:px-10">
                      ...
                      >
                        ...
                        >
                          <button
                            ...
                            className="block w-full min-w-0 truncate text-left text-ink-muted"
                          >
                            {col.prompt || "—"}
                          </button>
                          {expandedPromptIndex === col.index && (
                            <TRExpandedCellSurface>
                              <button
                                ...
                                className="block w-full whitespace-pre-wrap p-3 text-left text-xs leading-relaxed text-ink"
                              >
                                {col.prompt || "—"}
                              </button>
                            </TRExpandedCellSurface>
      ...
      <div className="mx-4 mb-2 min-h-0 min-w-0 flex-1 md:mx-8 md:mb-3">
        <div className={`h-full px-5 py-4 ${TABLE_SURFACE_CLASS}`}>
          <div className="space-y-3">
            <div className="h-3 w-24 animate-pulse rounded bg-surface-sunk" />
            <div className="h-3 w-5/6 animate-pulse rounded bg-surface-sunk" />
            <div className="h-3 w-3/4 animate-pulse rounded bg-surface-sunk" />
            <div className="h-3 w-4/5 animate-pulse rounded bg-surface-sunk" />
          </div>
          <div className="mt-8 space-y-3">
            <div className="h-3 w-28 animate-pulse rounded bg-surface-sunk" />
            <div className="h-3 w-11/12 animate-pulse rounded bg-surface-sunk" />
            <div className="h-3 w-2/3 animate-pulse rounded bg-surface-sunk" />
            <div className="h-3 w-10/12 animate-pulse rounded bg-surface-sunk" />
          </div>
          <div className="mt-8 space-y-3">
            <div className="h-3 w-20 animate-pulse rounded bg-surface-sunk" />
            <div className="h-3 w-4/6 animate-pulse rounded bg-surface-sunk" />
            <div className="h-3 w-5/6 animate-pulse rounded bg-surface-sunk" />
          </div>
        </div>
      </div>
