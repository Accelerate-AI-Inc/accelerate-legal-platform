        ...
        <div className="relative shrink-0" onClick={(e) => e.stopPropagation()}>
            <button
                ...
                className={`flex h-4 w-4 items-center justify-center rounded transition-colors ${
                    disabled
                        ...
                        : "text-ink-faint hover:bg-surface-sunk hover:text-ink-muted"
                }`}
            >
            ...
            {open &&
                ...
                createPortal(
                    <div
                        ref={panelRef}
                        className={`fixed z-[40] rounded-sm p-3 ${SURFACE_OVERLAY_CLASS}`}
                        style={{
                            top: menuPos.top,
                            left: menuPos.left,
                            width: menuPos.width,
                    ...
                    <input
                        ...
                        className={`w-full rounded-sm px-2 py-1 text-xs font-normal text-ink transition-colors focus:bg-surface/70 focus:outline-none ${INSET_FIELD}`}
                    />
                    ...
                    <div className="mt-3">
                        ...
                        <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                                <button
                                    id={`${menuId}-format`}
                                    className={`flex w-full items-center justify-between rounded-sm px-2 py-1 text-xs text-ink transition-colors hover:bg-surface/75 focus:outline-none ${INSET_FIELD}`}
                                >
                                ...
                                >
                                    {FORMAT_OPTIONS.map((o) => (
                                        <MenuRadioItem
                                            ...
                                            className="text-xs"
                                        >
                                            <o.icon className="h-3 w-3 text-ink-faint" />
                                            {o.label}
                                        </MenuRadioItem>
                                    ))}
                                </DropdownMenuRadioGroup>
                    ...
                    {format === "tag" && (
                        <div className="mt-2">
                            <div
                                className={`flex min-h-[28px] flex-wrap gap-1 rounded-sm px-2 py-1 transition-colors focus-within:bg-surface/70 ${INSET_FIELD}`}
                            >
                                {tags.map((tag, tagIdx) => (
                                    <span
                                        key={tag}
                                ...
                                <input
                                    ...
                                    className="min-w-[60px] flex-1 bg-transparent text-xs text-ink placeholder-ink-faint focus:outline-none"
                                />
                            </div>
                        </div>
                    )}
                    ...
                    <div className="mt-3">
                        <div className="flex items-start justify-between">
                            ...
                            <button
                                ...
                                className="inline-flex items-center gap-1 text-xs text-ink-muted transition-colors hover:text-ink disabled:text-ink-faint"
                            >
                                {generating ? (
                        ...
                        <textarea
                            ...
                            className={`w-full resize-none rounded-sm px-3 py-2 text-xs font-normal leading-relaxed text-ink placeholder-ink-faint transition-colors focus:bg-surface/70 focus:outline-none ${INSET_FIELD}`}
                        />
                    </div>
