...
export function DocVersionHistory({
            ...
            <>
                {Array.from({ length: skeletonCount }).map((_, index) => (
                    <div
                        ...
                        className="flex h-10 min-w-max items-center bg-surface-sunk pr-3"
                    >
                        <div
                            className={`sticky left-0 z-[60] ${DOC_NAME_COL_W} bg-surface-sunk py-2 pl-3 pr-2`}
                        ...
                        >
                            <div className="flex items-center">
                                ...
                                <div className="mr-2 h-4 w-4 shrink-0 rounded bg-surface-sunk animate-pulse" />
                                <div className="h-3 w-32 rounded bg-surface-sunk animate-pulse" />
                            </div>
                        </div>
    ...
    if (versions.length === 0) {
        return (
            <div className="flex items-center h-9 border-b border-rule text-xs text-ink-faint bg-paper/80">
                <div
                    className={`sticky left-0 z-[60] ${DOC_NAME_COL_W} bg-paper/80 py-2 pl-3 pr-2`}
                    style={treeNameCellStyle(depth)}
                >
    ...
    return (
        <>
            {ordered.map((v) => {
                ...
                const rowBg = isDeleted ? "bg-paper" : "bg-surface-sunk";
                const hoverBg = isDeleted ? "hover:bg-paper" : "hover:bg-surface-sunk";
                return (
                    <div
                        key={`ver-${docId}-${v.id}`}
                        onClick={() => {
                    ...
                    >
                        <div
                            className={`sticky left-0 z-[60] ${DOC_NAME_COL_W} ${rowBg} py-2 pl-3 pr-2 transition-colors ${
                                isDeleted ? "group-hover:bg-paper" : "group-hover:bg-surface-sunk"
                            }`}
                            style={treeNameCellStyle(depth)}
                        >
                            <div className="flex items-center">
                                ...
                                {isEditing ? (
                                    <input
                                        ...
                                        className="min-w-0 flex-1 border-b border-rule-strong bg-transparent text-xs text-ink outline-none focus:border-rule-strong"
                                    />
                                ) : (
                                    <span
                        ...
                        <div
                            className={`ml-auto w-20 shrink-0 truncate text-xs uppercase ${
                                isDeleted ? "text-ink-faint" : "text-ink-muted"
                            }`}
                        >
                            {versionFileType ?? (
                        ...
                        <div className="w-24 shrink-0 truncate text-xs text-ink-faint">
                            —
                        </div>
                        <div
                        ...
                        >
                            {dateLabel ? formatDate(v.created_at) : <span className="text-ink-faint">—</span>}
                        </div>
                        <div className="w-32 shrink-0 truncate text-xs text-ink-faint">
                            —
                        </div>
                        <div
