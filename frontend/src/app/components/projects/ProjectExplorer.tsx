...
export function ProjectExplorer({
    ...
    async function handleDropOnTarget(targetFolderId: string | null, e: React.DragEvent) {
        ...
        const movingFolderId = e.dataTransfer.getData("application/accelerate-folder");
    ...
    function isInternalDrag(e: React.DragEvent): boolean {
        return (
            Array.from(e.dataTransfer.types).includes("application/accelerate-doc") ||
            Array.from(e.dataTransfer.types).includes("application/accelerate-folder")
        );
    }
                ...
                {creatingIn === parentId && (
                    ...
                    >
                        ...
                        <input
                            ...
                            autoFocus
                            className="flex-1 min-w-0 text-xs bg-transparent outline-none border-b border-rule-strong text-ink"
                        ...
                        <li key={`f-${folder.id}`}>
                            <div
                                ...
                                className={`flex items-center gap-1.5 py-1.5 pr-2 rounded-sm cursor-pointer select-none transition-colors group ${
                                    isDragTarget
                                        ? "bg-accent-wash ring-1 ring-inset ring-accent"
                                        : "theme-dropdown-item"
                                }`}
                                style={{ paddingLeft: basePadding }}
                            ...
                            >
                                ...
                                {isRenaming ? (
                                    ...
                                    <span className="text-xs text-ink-muted truncate">{folder.name}</span>
                                )}
                            </div>
                            {isExpanded && (
                ...
                {childDocs.map((doc) => {
                    ...
                    return (
                        <li
                            ...
                            draggable
                            onDragStart={(e) => {
                                e.dataTransfer.setData("application/accelerate-doc", doc.id);
                                e.dataTransfer.effectAllowed = "move";
                            }}
                            ...
                            className={`flex items-center gap-2 py-1.5 pr-4 rounded-sm cursor-pointer select-none transition-colors ${
                                isSelected ? "theme-dropdown-selected text-ink" : "theme-dropdown-item text-ink-muted hover:text-ink"
                            }`}
                            style={{ paddingLeft: basePadding }}
                        >
    ...
    return (
        <ul
            className={`p-1 relative h-full ${dragOverRoot && dragOverFolderId === null ? "ring-2 ring-accent ring-inset" : ""}`}
            onContextMenu={(e) => {
        ...
        >
            ...
            {projectName && (
                ...
                >
                    ...
                    <span className="text-xs text-ink-muted truncate">{projectName}</span>
                </li>
            )}
            ...
            {documents.length === 0 && folders.length === 0 && creatingIn === undefined && (
                <li className="px-4 py-2 text-xs text-ink-faint">No documents in this project.</li>
            )}
            ...
            {contextMenu && (
                <div
                    ref={contextMenuRef}
                    className={`fixed z-50 w-44 overflow-hidden rounded-sm text-xs ${SURFACE_OVERLAY_CLASS}`}
                    style={{ top: contextMenu.y, left: contextMenu.x }}
                >
                    {onCreateFolder && (
                        <button
                            className="theme-dropdown-item flex w-full items-center gap-2 px-3 py-1.5 text-left text-ink"
                            onClick={() => {
                                setContextMenu(null);
                    ...
                    {contextMenu.folderId && onRenameFolder && (
                        <button
                            className="theme-dropdown-item w-full px-3 py-1.5 text-left text-ink"
                            onClick={() => {
                    ...
                    {contextMenu.folderId && onDeleteFolder && (
                        <button
                            className="flex w-full items-center gap-2 px-3 py-1.5 text-left text-critical hover:bg-critical-wash"
                            onClick={() => {
