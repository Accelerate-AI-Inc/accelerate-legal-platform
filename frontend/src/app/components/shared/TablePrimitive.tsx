...
export const CLOSE_ROW_ACTIONS_EVENT = "accelerate:close-row-actions";
...
export const TABLE_STICKY_CELL_BG = "bg-surface";
export const TABLE_PRIMARY_CELL_WIDTH_CLASS =
    "w-[248px] sm:w-[292px] md:w-[332px] shrink-0";
export const TABLE_CHECKBOX_CLASS =
    "mr-3 h-2.5 w-2.5 shrink-0 rounded border-rule cursor-pointer accent-ink";
    ...
    return (
        <DropdownMenu open={open} onOpenChange={setOpen}>
            <DropdownMenuTrigger asChild>
                <button
                    ...
                    className={`flex h-[18px] w-[22px] items-center justify-center rounded-sm transition-colors ${
                        value
                            ? `text-ink ${SURFACE_HOVER_CLASS} hover:text-ink`
                            : `text-ink-faint ${SURFACE_HOVER_CLASS} hover:text-ink`
                    }`}
                >
                    <ChevronDown
            ...
            >
                ...
                >
                    {allLabel}
                    {!value && <Check className="h-3.5 w-3.5 text-ink-faint" />}
                </MenuItem>
                {options.length > 0 && (
                    <DropdownMenuSeparator className="-mx-1 my-1 bg-surface/60" />
                )}
                {options.map((option) => {
...
export function SkeletonLine({ className }: { className?: string }) {
    return (
        <div
            className={cn("h-3 rounded bg-surface-sunk animate-pulse", className)}
        />
    );
}
...
export function SkeletonCheckbox({ className }: { className?: string }) {
    return (
        <div
            aria-hidden="true"
            className={cn(
                "mr-3 h-2.5 w-2.5 shrink-0 rounded bg-surface-sunk animate-pulse",
                className,
            )}
        />
    );
...
export function TableScrollArea({
    ...
    return (
        <div
            className={cn(
                "mx-4 mb-2 min-h-0 min-w-0 flex-1 rounded-sm md:mx-8 md:mb-3",
                className,
            )}
        >
            <div
...
export function TableHeaderRow({ children, className, ...props }: DivProps) {
    return (
        <div
            className={cn(
                "z-[70] flex h-10 min-w-max items-center bg-surface pr-3 text-xs font-medium text-ink-muted select-none",
                className,
            )}
            {...props}
        >
        ...
        <>
            <div
                className={cn(
                    ...
                    interactive && !selected && SURFACE_HOVER_CLASS,
                    selected && SURFACE_SELECTED_CLASS,
                    className,
                )}
    ...
    return (
        <div
            ...
            className={cn(
                ...
                !header && hover && SURFACE_GROUP_HOVER_CLASS,
                className,
            )}
        >
            {children}
...
export function TablePrimaryCell({
        ...
        label !== undefined ? (
            editing ? (
                <input
                    ...
                    onBlur={onEditCommit}
                    onClick={(e) => e.stopPropagation()}
                    className="min-w-0 flex-1 text-xs text-ink bg-transparent outline-none"
                />
            ) : (
    ...
    return (
        <TableStickyCell
            ...
            bgClassName={selected ? SURFACE_SELECTED_CLASS : bgClassName}
            className={className}
            hover={!selected}
        >
...
export function TableCell({ children, className, ...props }: DivProps) {
    return (
        <div
            className={cn("shrink-0 truncate text-xs text-ink-muted", className)}
            {...props}
        >
            {children}
        </div>
