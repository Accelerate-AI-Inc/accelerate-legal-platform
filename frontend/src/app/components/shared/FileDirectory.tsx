...
const DIRECTORY_CHECKBOX_CLASS =
    "h-2.5 w-2.5 shrink-0 justify-self-center cursor-pointer rounded border-rule accent-ink disabled:cursor-not-allowed";
    ...
    function renderDocumentRow(doc: Document, depth = 0) {
        ...
        return (
            <label
                ...
                className={`w-full rounded-sm ${DIRECTORY_GRID_CLASS} py-2 pr-2 text-left text-xs transition-all ${
          ...
          selected
            ...
            >
                ...
                <span
          className={`min-w-0 truncate ${selected ? "text-ink" : "text-ink"}`}
                >
                    {doc.filename}
                </span>
        ...
        docsInFolder.some((doc) => checkedIds.has(doc.id)) && !allSelected;
            ...
            return (
                <div key={folder.id}>
                    <div
                        style={{ paddingLeft: indentedRowPadding(depth) }}
                        className={`w-full rounded-sm ${DIRECTORY_GRID_CLASS} py-2 pr-2 text-xs transition-all ${OVERLAY_ROW_HOVER_CLASS}`}
                    >
                        <DirectorySelectionCheckbox
                            checked={allSelected}
            ...
            externalLoadingFolderIds.has(folder.id) ? (
              <Loader2 className="h-3.5 w-3.5 shrink-0 animate-spin text-ink-faint" />
                        ...
                        <span className="truncate text-ink-faint">-</span>
                ...
                <div className="flex min-h-0 flex-1 flex-col">
                    ...
                    <div className="min-h-0 flex-1 overflow-y-auto">
                        {[60, 45, 75, 55, 40].map((w, i) => (
                            <div
                                key={i}
                                className={`${DIRECTORY_GRID_CLASS} rounded-sm px-2 py-2`}
                            >
                                <Loader2 className="h-2.5 w-2.5 shrink-0 justify-self-center animate-spin text-ink-faint" />
                                <FileTypeIcon
                                    fileType={null}
                                    muted
                ...
                const projectSelectionReady =
                  ...
                  !allProjectDocsSelected;
                            return (
                                <div key={project.id}>
                                    <div
                                        className={`w-full rounded-sm ${DIRECTORY_GRID_CLASS} px-2 py-2 text-xs transition-all text-left ${OVERLAY_ROW_HOVER_CLASS}`}
                                    >
                                        <DirectorySelectionCheckbox
                      ...
                      <span className="truncate text-ink-faint">-</span>
                        ...
                        {loadingProjectLevels.has(`${project.id}:root`) ? (
                          <p className="flex items-center gap-2 pl-7 py-2 text-xs text-ink-faint">
                            <Loader2 className="h-3.5 w-3.5 animate-spin" />
                            Loading project files
                          </p>
                        ) : docs.length === 0 && projectFolders.length === 0 ? (
                                                <p className="pl-7 py-1 text-xs text-ink-faint">
                                                    Empty
                                                </p>
                                            ) : (
                                                <>
...
function FileDirectoryHeader() {
    return (
        <div
            className={`${DIRECTORY_GRID_CLASS} px-2 pb-1 pt-0.5 text-[11px] font-medium text-ink-faint`}
        >
...
function FileDirectoryMetaCells({
    ...
    return (
        <>
            <span className="truncate text-ink-faint">{version ?? "--"}</span>
            <span className="truncate text-ink-faint">{created ?? "--"}</span>
      <span className="truncate text-right text-ink-faint">{size ?? "--"}</span>
        </>
    );
}
