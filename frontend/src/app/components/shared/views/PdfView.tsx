...
import { SURFACE_VEIL_CLASS } from "@/shared/ui/SurfaceUI";
...
export function PdfView({
    ...
    return (
        <div
            className={`relative flex flex-col bg-surface-sunk flex-1 overflow-hidden ${rounded ? "rounded-sm" : ""}`}
        >
            <div
            ...
            {numPages > 0 && (
                <>
                    ...
                    <div className="absolute bottom-4 left-4 pointer-events-none">
                        <span className={`flex items-center rounded-full px-3 py-1.5 text-xs font-medium tabular-nums text-ink ${SURFACE_VEIL_CLASS}`}>
                            {currentPage}/{numPages}
                        </span>
                    </div>

                    {/* Zoom controls — bottom right */}
                    <div className={`absolute bottom-4 right-4 flex items-center gap-px rounded-full px-1 py-1 ${SURFACE_VEIL_CLASS}`}>
                        <button
                            onClick={handleZoomOut}
                            disabled={zoom <= ZOOM_MIN}
                            className="flex items-center justify-center w-7 h-7 rounded-full text-ink-muted hover:bg-surface/80 disabled:opacity-30 transition-colors"
                        >
                            <ZoomOut className="h-3.5 w-3.5" />
                        </button>
                        <span className="text-xs font-medium text-ink-muted tabular-nums w-9 text-center select-none">
                            {Math.round(zoom * 100)}%
                        </span>
                        <button
