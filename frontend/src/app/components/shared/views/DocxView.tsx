...
export function DocxView({
    ...
    return (
        <div
            className={`relative flex flex-col flex-1 overflow-hidden bg-surface-sunk ${rounded ? "rounded-sm" : ""}`}
        >
            {warning && (
                <div className="absolute top-2 left-2 z-10 flex items-center gap-2 rounded-sm border border-caution bg-caution-wash px-2 py-1 text-xs text-caution">
                    <span>{warning}</span>
                    <button
                        type="button"
