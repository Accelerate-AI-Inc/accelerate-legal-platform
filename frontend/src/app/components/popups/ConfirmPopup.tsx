...
export function ConfirmPopup({
    ...
    return createPortal(
        <div className="pointer-events-none fixed inset-x-0 bottom-5 z-[230] flex justify-center px-4">
            <div
                className={cn(
                    `pointer-events-auto w-[min(92vw,520px)] rounded-sm px-4 py-3 text-sm ${SURFACE_OVERLAY_CLASS}`,
                    className,
                )}
            >
                {title && (
                ...
                {message && (
                    <div
                        className={cn("text-xs text-ink", title && "mt-1")}
                    >
                        {message}
                    </div>
                )}
                <div className="mt-3 flex items-center justify-end gap-2">
                    ...
                    <PillButton
                        tone={isDeleteAction ? "critical" : "ink"}
                        size="sm"
                        onClick={onConfirm}
