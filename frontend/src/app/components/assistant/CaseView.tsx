...
export function CaseView({
    ...
    const surfaceClassName = SURFACE_PANEL_CLASS;
        ...
        <div className="flex min-h-0 flex-1 flex-col">
                        ...
                        {subdocuments.map((subdocument) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`document-tab group relative flex h-8 max-w-[180px] shrink-0 items-center rounded-t-sm px-3 font-serif text-[13px] transition-colors ${isActive ? "z-20" : "z-10"}`}
                                >
            ...
            <div className="flex min-h-0 flex-1 flex-col">
                ...
                {visibleError && (
                    ...
                    >
                        <p className="font-serif text-sm text-critical">
                            {visibleError}
                        </p>
                        {onRetry && (
                            <PillButton tone="paper" onClick={onRetry}>
                                Try again
                            </PillButton>
                        )}
                    </div>
                ...
                {!visibleLoading &&
                    ...
                    subdocuments.length === 0 && (
                        <p
                            className={cn(
                                "p-4 font-serif text-sm text-ink-muted",
                                surfaceClassName,
                            )}
                        >
...
function CaseSubdocument({
    ...
    return (
        <article ref={contentRef} className="case-document-content pb-6">
            ...
            {sanitizedHtml ? (
                <div
                    className="prose prose-sm max-w-none font-serif leading-7 text-ink [&_*]:font-serif [&_.case-page-number]:mx-1 [&_.case-page-number]:text-xs [&_.case-page-number]:text-ink-faint [&_a]:text-accent [&_a]:underline [&_a:hover]:text-accent [&_p]:my-3"
                    dangerouslySetInnerHTML={{ __html: sanitizedHtml }}
                />
            ) : (
                <div className="whitespace-pre-wrap font-serif text-sm leading-7 text-ink [&_p]:my-3">
