...
import { listWorkflows } from "@/app/lib/accelerateApi";
import { userFacingApiError } from "@/app/lib/userFacingError";
import { OVERLAY_ROW_HOVER_CLASS } from "@/app/components/ui/surface";
    ...
    >
      {screen === "list" ? (
        <div className="flex min-h-0 flex-1 flex-col pb-5">
          ...
          >
            {error && (
              <p className="px-3 py-2 text-xs text-critical" role="alert">
                {error}
              </p>
            )}
            {filteredActions.length === 0 ? (
              ...
              filteredActions.map((action) => (
                <button
                  ...
                  className={`flex w-full items-center gap-3 rounded-sm px-3 py-2.5 text-left text-sm text-ink transition-colors ${OVERLAY_ROW_HOVER_CLASS}`}
                >
  ...
  return (
    <div className="flex items-center justify-between gap-4">
      <div>
        ...
        <p className="mt-1 text-xs leading-relaxed text-ink-faint">{caption}</p>
      </div>
      <ToggleSwitch
        checked={checked}
