...
describe("ModelToggle responsive trigger", () => {
    ...
    it("uses the Settings2 icon in a compact chat input", () => {
        ...
        expect(trigger).toHaveClass("w-8", "rounded-sm");
    ...
    it("does not add its own elevation to the selected model row", async () => {
        const user = userEvent.setup();
        render(
            <ModelToggle
                value="gemini-3-flash-preview"
        ...
        expect(selectedRow).toHaveClass("theme-dropdown-item", "text-ink");
        expect(selectedRow).toHaveClass("rounded-sm");
        expect(selectedRow).toHaveAttribute("data-selected", "true");
        // Elevation belongs to the material classes; a menu row may not own one.
