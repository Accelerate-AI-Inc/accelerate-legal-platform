...
import { RESPONSE_SURFACE_CLASS } from "./message/messageStyles";
import {
    SURFACE_HOVER_CLASS,
    SURFACE_INSET_CLASS,
...
const QUOTE_CARD_SURFACE = "rounded-sm bg-surface-sunk";
...
export function CitationQuotesSection({
    ...
    return (
        <div className="px-2 pb-2">
            <div className={`${RESPONSE_SURFACE_CLASS} p-2`}>
                <div className="mb-2 flex items-center justify-between">
                    <ContextNumberBadge number={citationRef} label="Citation" />
                    <div className="ml-auto flex items-center gap-2">
                        {hasMultipleQuotes && (
                            <div className="flex items-center gap-1">
                                <span className="mr-0.5 text-xs font-medium text-ink-muted">
                                    Quotes
                                </span>
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`flex h-4 w-4 items-center justify-center rounded-full text-[9px] transition-colors ${
                                                currentIndex === index &&
                                                !isUnverified
                                                    ? "!bg-accent-wash !text-accent-wash font-medium dark:!bg-accent-wash dark:!text-paper"
                                                    : isUnverified
                                                      ? "cursor-not-allowed !bg-critical-wash/85 !text-critical dark:!bg-critical-wash dark:!text-paper"
                                                      : "bg-surface-sunk/80 text-ink hover:bg-surface-sunk hover:text-ink"
                                            }`}
                                        >
                                            {index + 1}
                                        </button>
...
function RelevantQuoteSkeleton() {
    return (
        <div className={`animate-pulse px-3 py-2 ${QUOTE_CARD_SURFACE}`}>
            <div className="h-3 w-28 rounded bg-surface-sunk" />
            <div className="mt-2.5 h-3 w-full rounded bg-surface-sunk" />
            <div className="mt-2 h-3 w-11/12 rounded bg-surface-sunk" />
        ...
        <div className={`px-3 py-2 ${QUOTE_CARD_SURFACE}`}>
            <p
                className={`font-serif text-sm leading-6 ${
                    tone === "error" ? "text-critical" : "text-ink-muted"
                }`}
            >
                {children}
            </p>
        ...
        <div>
            ...
            <div className="mt-2 flex items-center justify-between gap-2">
                <CiteButton
                    ...
                    className={`h-6 rounded-full px-2 text-ink-muted ${SURFACE_INSET_CLASS} ${SURFACE_HOVER_CLASS}`}
                    showText
                />
                <PillButton
                    tone="ink"
