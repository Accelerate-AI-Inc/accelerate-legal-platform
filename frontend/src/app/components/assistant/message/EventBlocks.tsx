...
import { API_BASE } from "@/app/lib/accelerateApi";
...
import { RESPONSE_SURFACE_CLASS, withoutMarkdownNode } from "./messageStyles";
...
function EventConnector() {
    return (
        <div className="absolute w-[1px] bg-rule-strong top-[14px] left-[3px] translate-x-[-50%] h-[calc(100%+10px)]" />
    );
}
    ...
    const dotColorClass =
        dotColor === "green"
            ...
            : dotColor === "red"
              ...
              : "bg-ink-faint";
    return (
        <div className="flex items-start text-sm font-serif text-ink-muted relative">
            {showConnector && <EventConnector />}
            {isStreaming ? (
                <div className="mt-2 w-1.5 h-1.5 shrink-0 rounded-full border border-rule-strong border-t-transparent animate-spin" />
            ) : (
                <div
    ...
    return (
        ...
        >
            <button
                ...
                className="flex items-center text-sm font-serif text-ink-muted hover:text-ink-muted transition-colors"
            >
            ...
            {showContent && (
                <div className="mt-2">
                    ...
                    >
                        <div
                            ref={contentRef}
                            className="text-sm font-serif text-ink-faint prose prose-sm max-w-none [&>*]:text-ink-faint [&>*]:text-sm"
                        >
                            <ReactMarkdown
                        ...
                        {isCollapsed && (
                            <>
                                ...
                                <button
                                    ...
                                    className="absolute left-1/2 bottom-2 z-10 -translate-x-1/2 text-ink-faint transition-colors hover:text-ink-muted"
                    ...
                    {isOverflowing && isContentOpen && isExpanded && (
                        <button
                            ...
                            className="mx-auto mt-2 flex text-ink-faint transition-colors hover:text-ink-muted"
...
export function DocDownloadBlock({
        ...
        <div className="flex items-center gap-3 px-4 py-3 min-w-0 flex-1">
            ...
            <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2 min-w-0">
                    <p className="text-lg font-serif text-ink text-wrap">
                        {basename}
                    </p>
                    {hasVersion && (
                        <span className="shrink-0 inline-flex items-center rounded-sm border border-rule bg-surface px-1.5 py-0.5 text-[10px] font-medium text-ink-muted">
                            V{versionNumber}
                        </span>
                    )}
                </div>
    ...
    const downloadIcon = spinning ? (
        <div
            aria-disabled
            className="shrink-0 flex items-center bg-surface/25 px-6 text-ink-faint cursor-not-allowed"
        >
        ...
        <button
            ...
            className="shrink-0 flex items-center bg-surface/25 px-6 text-ink-muted transition-colors hover:bg-surface/55 hover:text-ink cursor-pointer"
        >
            <Download size={13} />
        </button>
    );
    ...
    if (onOpen) {
        return (
            <div
                className={`flex items-stretch overflow-hidden w-full font-serif ${RESPONSE_SURFACE_CLASS}`}
            >
                <button
                    type="button"
                    onClick={onOpen}
                    className="flex items-stretch flex-1 min-w-0 text-left transition-colors hover:bg-surface/45 cursor-pointer"
                >
                    {body}
                </button>
                {downloadIcon}
    ...
    return (
        <EventBlock showConnector={showConnector} dotColor="green">
            ...
            {onClick ? (
                <button
                    onClick={onClick}
                    className="text-left hover:text-ink transition-colors cursor-pointer"
                >
                    {title}
                </button>
            ) : (
        ...
        >
            {hasItems ? (
                <button
                    ...
                    className="text-left hover:text-ink transition-colors inline-flex items-center"
                >
                    <span className="font-medium">{label}</span>
            ...
            {isOpen && hasItems && (
                <ul className="mt-2 flex flex-col gap-1 text-sm font-serif text-ink-muted">
                    {items!.map((item, idx) => {
