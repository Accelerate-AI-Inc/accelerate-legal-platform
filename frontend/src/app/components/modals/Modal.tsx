...
function ModalActionButton({
  ...
  if (fallbackVariant === "cancel") {
    return (
      <button
        type="button"
        className="px-2 py-1.5 text-sm text-ink-muted transition-colors hover:text-ink disabled:cursor-not-allowed disabled:opacity-40"
        {...props}
      >
        {label}
      </button>
  ...
  // `variant` is the modal's own vocabulary; PillButton speaks in tones.
  const tone =
    variant === "danger"
      ? "critical"
      : variant === "blue"
