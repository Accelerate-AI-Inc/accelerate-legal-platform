...
export function AddDocumentsModal({
    ...
    return (
        ...
        >
            ...
            {uploadWarning && (
                <div className="mb-2 flex items-center gap-2 rounded-sm border border-critical bg-critical-wash px-3 py-2 text-xs text-ink">
                    <AlertCircle className="h-3.5 w-3.5 shrink-0 text-critical" />
                    <span className="min-w-0 flex-1">{uploadWarning}</span>
                    <button
                        type="button"
                        onClick={() => setUploadWarning(null)}
                        className="shrink-0 rounded p-0.5 text-ink hover:bg-surface-sunk"
