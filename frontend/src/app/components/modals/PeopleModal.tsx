...
import type { ProjectPeople } from "@/app/lib/accelerateApi";
...
/**
 * Roster of every Accelerate Legal member with access to the project, with controls to
                ...
                {onSharedWithChange && (
                    <section className="space-y-2">
                        <AddUserInput
                            ...
                            className="bg-surface focus-within:bg-surface"
                        />
                        {error && (
                    ...
                    {rosterPending ? (
                        <div className="min-h-0 flex-1 space-y-1">
                            {[1, 2].map((item) => (
                                ...
                                >
                                    <div className="h-6 w-6 shrink-0 animate-pulse rounded-full bg-surface-sunk" />
                                    <div className="min-w-0 flex-1">
                                        <div className="h-3 w-40 animate-pulse rounded bg-surface-sunk" />
                                    </div>
                                    <div className="h-4 w-12 shrink-0 animate-pulse rounded-full bg-surface-sunk" />
                                </div>
                            ))}
                        </div>
                    ) : roster.length === 0 ? (
                            ...
                            {roster.map((entry) => {
                                ...
                                return (
                                    <li
                                        ...
                                        className={`${OVERLAY_ROW_HOVER_CLASS} group relative flex items-center gap-2.5 rounded-sm px-2 py-1.5 transition-colors`}
                                    >
                                        <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full border border-rule/80 bg-surface text-ink">
                                            {initial ? (
                                        ...
                                        {entry.role === "owner" && (
                                            <span className="shrink-0 rounded-full px-2 py-1 text-xs text-ink-faint">
                                                Owner
                                            </span>
                                        )}
                                        {entry.role === "member" && (
                                            ...
                                            >
                                                <span className="rounded-full px-2 py-1 text-xs text-ink-faint">
                                                    Member
                                                </span>
                                                {onSharedWithChange && (
                                                    <>
                                                        <button
                                                            ...
                                                            className={`flex h-6 items-center justify-center overflow-hidden rounded-full text-xs leading-none text-ink-muted transition-all hover:bg-surface-sunk/70 hover:text-ink disabled:opacity-50 ${
                                                                memberMenuEmail ===
                                                                entryEmail
                                                                    ? "w-6 opacity-100"
                                                        ...
                                                        {memberMenuEmail ===
                                                            entryEmail && (
                                                            <div
                                                                className={`absolute right-0 top-full z-30 mt-1 min-w-28 overflow-hidden rounded-sm p-1 ${SURFACE_OVERLAY_CLASS}`}
                                                                onClick={(
                                                                    event,
                                                                ) =>
                                                                    event.stopPropagation()
                                                            ...
                                                            >
                                                                <button
                                                                    ...
                                                                    className="flex w-full items-center gap-2 rounded-sm px-3 py-1.5 text-left text-xs text-critical transition-colors hover:bg-critical/10 disabled:opacity-50"
                                                                >
                                                                    {isRemoving && (
