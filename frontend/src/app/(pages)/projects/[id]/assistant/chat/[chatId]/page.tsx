    ...
    return (
        <div className="flex-1 flex items-center justify-center">
            <div className="relative flex items-center justify-center h-[28px]">
                ...
                <h1
                    ref={textRef}
                    className="absolute text-3xl font-serif font-light text-ink whitespace-nowrap"
                    style={{
                        left: "50%",
                        transform: loaded
...
function Divider({ onDrag }: { onDrag: (dx: number) => void }) {
    ...
    return (
        <div className="relative w-0 shrink-0 z-10">
            ...
            >
                {isDragging && (
                    <div className="w-1 bg-accent transition-colors" />
                )}
            </div>
        </div>
    );
...
export default function ProjectAssistantChatPage({ params }: Props) {
    ...
    const handleChatDrop = (e: React.DragEvent) => {
        e.preventDefault();
        const docId = e.dataTransfer.getData("application/accelerate-doc");
        if (!docId) return;
    ...
    return (
        <div className="flex flex-col h-full">
            ...
            {/* Three-panel body */}
            <div className="flex flex-1 min-h-0 border-t border-rule overflow-hidden">
                {/* LEFT: Project Explorer */}
                {!explorerCollapsed && (
                    <>
                        <div
                            style={{ width: explorerWidth }}
                            className="shrink-0 flex flex-col border-r border-rule"
                            onDragOver={(e) => {
                                e.preventDefault();
                                // Only show the upload overlay for external file drags, not internal moves
                                const isInternal =
                                    Array.from(e.dataTransfer.types).includes(
                                        "application/accelerate-doc",
                                    ) ||
                                    Array.from(e.dataTransfer.types).includes(
                                        "application/accelerate-folder",
                                    );
                        ...
                        >
                            {/* Explorer header */}
                            <div className="h-10 flex items-center justify-between px-3 border-b border-rule shrink-0">
                                ...
                                <div className="flex items-center gap-1">
                                    ...
                                    <button
                                        ...
                                        className="p-1 rounded text-ink-faint hover:text-ink hover:bg-surface-sunk transition-colors disabled:opacity-40"
                                    >
                                        {uploading ? (
                                    ...
                                    <button
                                        ...
                                        className="p-1 rounded text-ink-faint hover:text-ink hover:bg-surface-sunk transition-colors"
                                    >
                            ...
                            {/* Drop overlay */}
                            <div
                                className={`flex-1 overflow-y-auto relative h-full ${explorerDragOver ? "bg-accent-wash" : ""}`}
                                onDragOver={(e) => {
                                    e.preventDefault();
                                }}
                                onDrop={async (e) => {
                ...
                {explorerCollapsed && (
                    <div className="shrink-0 flex flex-col border-r border-rule">
                        <div className="h-10 flex items-center justify-center border-b border-rule shrink-0 px-1">
                            <button
                ...
                <div className="flex-1 flex flex-col min-w-0 border-r border-rule">
                    {/* Tab bar */}
                    <div
                        ref={tabBarRef}
                        className="h-10 flex items-end border-b border-rule shrink-0 overflow-x-auto min-w-0 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden"
                    >
                        {tabs.length === 0 ? (
                            <span className="px-4 self-center text-xs text-ink">
                                Document Viewer
                            </span>
                        ) : (
                            tabs.map((tab) => {
                                ...
                                return (
                                    <div
                                        ...
                                        className={`group flex items-center gap-1.5 px-3 h-full border-r border-rule cursor-pointer shrink-0 max-w-[260px] transition-colors ${
                                            isActive
                                                ? "bg-surface-sunk"
                                    ...
                                    >
                                        ...
                                        />
                                        <span
                                            className={`text-xs truncate ${isActive ? "text-ink font-medium" : "text-ink-muted"}`}
                                        >
                                            {tab.filename}
                                        </span>
                                        {showVersionBadge && (
                                        ...
                                        <button
                                            ...
                                            className={`shrink-0 transition-colors ${isActive ? "text-ink-muted hover:text-ink" : "text-ink-faint hover:text-ink-muted"}`}
                                        >
                                            <X className="h-3 w-3" />
                                        </button>
                                    </div>
                    ...
                    <div className="flex-1 min-h-0 overflow-hidden flex flex-col">
                        {activeTab ? (
                            ...
                            <div className="flex items-center justify-center h-full px-8 bg-surface-sunk">
                ...
                >
                    <div className="h-10 flex items-center px-4 border-b border-rule shrink-0">
                        <span className="text-xs text-ink">
                            Project Assistant
                        </span>
                    </div>
                    ...
                    {!chatLoaded ? (
                        <div className="flex-1 px-4 py-4 space-y-4">
                            <div className="flex justify-end">
                                <div className="bg-surface-sunk rounded-sm p-4 w-3/4">
                    ...
                    <div className="absolute bottom-2 left-0 right-0 z-30 w-full md:bottom-3">
                        <div className="pointer-events-none absolute -bottom-2 left-4 right-4 z-0 h-7 bg-paper md:-bottom-3" />
