        ...
        await runSensitiveAction({ type: "create" }, async () => {
            ...
            try {
                ...
                try {
                    ...
                    if (
                        err instanceof AccelerateApiError &&
        ...
        <div>
            <div className="mb-4">
                <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="flex shrink-0 items-center rounded-full border border-rule/70 bg-surface p-0.5">
                        <button
                            type="button"
                            onClick={() => setAddOpen(true)}
                            className={`flex h-6 items-center justify-center gap-1 rounded-full px-2.5 text-xs font-medium text-ink-muted transition-colors hover:text-ink ${SURFACE_HOVER_CLASS} ${SURFACE_PRESSED_CLASS}`}
                        >
            ...
            {error && (
                <div className="mb-4 rounded-sm bg-critical-wash px-3 py-2 text-sm text-critical">
                    {error}
                </div>
            )}
...
function ConnectorRow({
    ...
    return (
        <SettingsSection>
            <div
                className="cursor-pointer rounded-sm px-4 py-3 transition-colors hover:bg-surface/70"
                role="button"
                tabIndex={0}
                onClick={onOpen}
                onKeyDown={(event) => {
            ...
            >
                <div className="grid grid-cols-[minmax(0,1fr)_auto] gap-x-3 gap-y-3">
                    ...
                    <button
                        ...
                        className="shrink-0 justify-self-end text-xs font-medium text-ink-muted transition-colors hover:text-ink"
                    >
                        Details
                    </button>
                </div>
                        ...
                        <div className="mb-2 flex items-center justify-between">
                            ...
                            <div className="flex items-center">
                                <button
                                    ...
                                    className="inline-flex items-center gap-1 text-xs font-medium text-ink-muted transition-colors hover:text-ink disabled:cursor-not-allowed disabled:text-ink-faint"
                                >
                                    {busyKey === `refresh:${connector.id}` ? (
                    ...
                    <div className="relative">
                        ...
                        {tokenAction && (
                            <button
                                ...
                                className={`absolute inset-y-1 right-1.5 px-1 text-xs font-medium transition-colors disabled:cursor-not-allowed disabled:text-ink-faint ${
                                    tokenAction.active || tokenAction.cleared
                                        ? "text-critical hover:text-critical"
...
function ToolListSkeleton({
    ...
    return (
        <div
            className={`overflow-hidden rounded-sm border border-rule bg-surface/60 ${
                ...
                {Array.from({ length: rowCount }).map((_, index) => (
                    <div key={index} className="px-3 py-2">
                        <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-2">
                            ...
                            <div className="h-3.5 w-full max-w-[220px] animate-pulse rounded bg-surface-sunk" />
                            <div className="h-4 w-7 animate-pulse rounded-full bg-surface-sunk" />
                        </div>
                    </div>
                ))}
            </div>
...
function ScrollableToolList({
    ...
    if (connector.tools.length === 0) {
        return (
            <div
                className={`rounded-sm bg-paper px-3 py-3 text-sm text-ink-muted ${
                    fill ? "min-h-0 flex-1" : ""
                }`}
            >
    ...
    return (
        <div
            className={`overflow-y-auto rounded-sm border border-rule bg-surface/60 ${
                fill ? "min-h-0 flex-1" : "max-h-72"
            }`}
        >
            <div>
                        ...
                        <div key={tool.id} className="px-3 py-2">
                            <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-2">
                                <button
                                    ...
                                    className="inline-flex h-5 w-5 items-center justify-center text-ink-faint transition-colors hover:text-ink"
                                    aria-label={`${
