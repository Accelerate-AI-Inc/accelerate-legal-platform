...
import { type ApiKeyState } from "@/app/lib/accelerateApi";
import {
    MODELS,
    SETTINGS_MODELS,
    canonicalModelId,
        ...
        <DropdownMenu onOpenChange={setIsOpen}>
            <DropdownMenuTrigger asChild>
                <button
                    type="button"
                    disabled={isSaving}
                    className={`flex h-9 items-center justify-between gap-2 hover:bg-surface-sunk/70 ${SETTINGS_CONTROL_CLASS}`}
                >
                    ...
                    {isSaving ? (
                        ...
                        <ChevronDown
                            className={`h-3.5 w-3.5 shrink-0 text-ink-muted transition-transform duration-200 ${isOpen ? "rotate-180" : ""}`}
                        />
                    )}
                </button>
            </DropdownMenuTrigger>
            ...
            >
                ...
                >
                    ...
                    {!value && (
                        <Check className="h-3.5 w-3.5 text-ink-muted ml-1" />
                    )}
                </MenuItem>
                ...
                {availableGroups.map(({ group, items }, groupIndex) => {
                    return (
                        <div key={group}>
                            ...
                            <DropdownMenuLabel className="text-[10px] uppercase tracking-wider text-ink-faint">
                                {group}
                            </DropdownMenuLabel>
                            {items.map((m) => {
                                return (
