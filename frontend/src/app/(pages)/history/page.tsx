...
import { HistoryIcon } from "@/app/components/shared/AppSidebarIcons";
...
const STATUS_DOT_STYLES: Record<string, string> = {
  completed: "bg-positive/80",
  cancelled: "bg-caution/80",
  failed: "bg-critical/80",
};

const STATUS_TEXT_STYLES: Record<string, string> = {
  completed: "text-positive",
  cancelled: "text-caution",
  failed: "text-critical",
};

const SURFACE_DOT =
  "h-2.5 w-2.5 shrink-0 rounded-full border border-rule/80";
      ...
      >
        {initialLoading ? (
          ...
          <TableBody className="flex">
            <TableEmptyState>
              <EmptyState
                ...
                description="Your activity could not be loaded."
                action={
                  <PillButton
                    tone="paper"
          ...
          <TableBody className="flex">
            <TableEmptyState>
              <EmptyState
                icon={<HistoryIcon data-testid="history-empty-icon" />}
                title="No history yet"
                description="Actions appear here as you use the app."
              />
            </TableEmptyState>
          </TableBody>
        ) : (
            ...
            {events.map((event) => {
              ...
              return (
                <TableRow key={event.id} interactive={false}>
                  ...
                  <TableCell className="w-72 pr-6 text-xs text-ink">
                    {href ? (
                  ...
                  <TableCell className="w-28 text-xs">
                    <span
                      className={`inline-flex items-center gap-2 ${STATUS_TEXT_STYLES[event.status] ?? "text-ink-muted"}`}
                    >
                      <span
                        aria-hidden="true"
                        data-testid={`status-dot-${event.id}`}
                        className={`${SURFACE_DOT} ${STATUS_DOT_STYLES[event.status] ?? "bg-rule-strong/80"}`}
                      />
                      {formatStatus(event.status)}
                    </span>
                  </TableCell>
                  <TableCell className="w-44 text-xs text-ink">
...
const HISTORY_DATE_PICKER_CLASS_NAMES = {
  ...dayPickerStyles,
  root: cn(dayPickerStyles.root, "m-0 w-56 text-xs text-ink"),
  caption_label: cn(
  ...
  button_previous: cn(
    ...
    "rounded-full text-ink-muted hover:bg-surface/70 hover:text-ink",
  ),
  button_next: cn(
    dayPickerStyles.button_next,
    "rounded-full text-ink-muted hover:bg-surface/70 hover:text-ink",
  ),
  chevron: cn(dayPickerStyles.chevron, "h-3.5 w-3.5 fill-ink-muted"),
  weekday: cn(
    dayPickerStyles.weekday,
    "!p-0 !text-[10px] !font-medium text-ink-faint",
  ),
  day_button: cn(
    dayPickerStyles.day_button,
    "text-xs font-normal hover:bg-surface/80 focus-visible:outline-2 focus-visible:outline-offset-1 focus-visible:outline-rule-strong",
  ),
  selected: cn(
    dayPickerStyles.selected,
    "!text-xs !font-medium [&>button]:!bg-surface-sunk [&>button]:!text-ink",
  ),
  today: cn(dayPickerStyles.today, "font-semibold text-ink"),
};
