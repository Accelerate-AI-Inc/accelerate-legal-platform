...
describe("HistoryPage", () => {
  ...
  it("renders the shared page header, toolbar, and table controls", async () => {
    ...
    expect(screen.getByText("Completed")).toHaveClass("text-positive");
  ...
  it("uses a liquid range calendar and exports the selected dates", async () => {
    ...
    expect(screen.getByRole("menu")).toHaveClass("surface-overlay");
  ...
  it("shows the history clock in the empty table placeholder", async () => {
    ...
    // The shared EmptyState primitive owns the left-aligned composition, and
    // TableEmptyState owns the column it sits in.
    const emptyState = screen
      .getByText("No history yet")
      .closest('[data-slot="empty-state"]');
    expect(emptyState).toHaveClass("items-start", "text-left");
    expect(emptyState?.parentElement).toHaveClass("max-w-[260px]");
    // A line icon rendered inline, not a raster asset fetched by URL.
    expect(screen.getByTestId("history-empty-icon").tagName.toLowerCase()).toBe(
      "svg",
    );
  });
