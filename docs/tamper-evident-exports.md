...
Accelerate Legal hashes a document version's bytes with SHA-256 whenever it writes them.
...
1. The bytes `accelerate-project-manifest-v1`.
2. A NUL byte.
3. The manifest digest bytes.

Manifests exported before the product rename were signed over the context
`mike-project-manifest-v1` instead. Nothing signs with it any more, but a
verifier must accept both — `LEGACY_SIGNING_CONTEXTS` in
`backend/src/lib/manifestSigning.ts` is the authoritative list.
...
crypto.verify(
  null,
  Buffer.concat([
    Buffer.from("accelerate-project-manifest-v1\0"),
