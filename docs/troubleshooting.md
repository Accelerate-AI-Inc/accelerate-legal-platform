...
Supabase dashboard. Accelerate Legal intentionally shows the same password-reset response
...
Another local Postgres or Supabase stack is using Accelerate Legal's default host port.
...
backend can reach the URL configured by `OLLAMA_BASE_URL`. Refresh Accelerate Legal after
installing a model.
