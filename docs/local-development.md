...
- the Accelerate Legal frontend and backend;
...
| --- | --- | --- |
| Accelerate Legal | `http://localhost:3000` | Main application |
...
Accelerate Legal backend does not send it directly.
...
by `ollama list` appears in Accelerate Legal's model pickers under **Local**, without an API
key.
...
Accelerate Legal. Replace `MODEL_TAG` with a tag from the Ollama library:
...
rejects tools, Accelerate Legal retries without them so plain chat can continue. Model size
