...
Use this path when connecting Accelerate Legal to managed Supabase and S3-compatible
...
The job resolves `ACCELERATE_WORKFLOWS_REF`, downloads and validates the raw
`accelerate-ai/accelerate-workflows` archive, uploads reference assets to the
configured S3-compatible storage, and transactionally replaces the active
`accelerate_workflows` catalog. Temporary archive and JSON files are deleted when the
...
The resources catalog ships as a committed dataset
(`backend/src/lib/resources/legalAiResources.json`) rather than being fetched at
deploy time. Load it into the database from the built backend artifact:
...
The job transactionally replaces the active `legal_ai_resources` catalog. Rows
are keyed on `(slug, content_hash)`, so re-running with an unchanged dataset is
a no-op. The Resources page and the assistant's `legal_resource_lookup` tool
both fall back to the bundled dataset when the table is empty, so this job is
not required for a working install — it is what makes server-side search and
filtering available.

To refresh the dataset from upstream, run
`node scripts/import-legal-ai-resources.mjs` and commit the regenerated JSON in
both apps, then run the sync job. See
`backend/src/lib/resources/ATTRIBUTION.md`.

### Upgrading from a pre-rename deployment

`R2_BUCKET_NAME` must stay set to its existing value. The default changed with
the product name, so an upgrade that relies on the default would point at a new,
empty bucket and orphan every stored object.

Everything else migrates itself: the session cookie, the BYOK and MCP
encryption salts, the export manifest signing context, and the Word add-in's
document-settings keys all read their pre-rename spellings and rewrite on next
use. Re-run `npm run sync:workflows` after upgrading so catalog assets land
under the current storage prefix.
...
- the internal Accelerate Legal backend origin for frontend `API_BASE_URL`.
...
Configure production SMTP in the Supabase dashboard; Accelerate Legal does not require a
...
```text
https://your-accelerate.example/auth/callback
```
...
provider page, not Accelerate Legal's frontend callback. For hosted Supabase it normally
has this form:
...
both deployed Accelerate Legal clients:
...
```bash
docker build -t accelerate-word-addin \
