...
reference for the [Accelerate Legal Word add-in](../word-addin/README.md).

## Architecture

The add-in uses the same backend-managed session and Accelerate Legal API as the web app:

- Password, Google, refresh, and logout operations go through the Accelerate Legal backend.
- Chat, workflows, uploads, profiles, and model discovery use the Accelerate Legal API.
- Word conversations use dedicated `word_*` tables and do not appear in the
  web assistant's normal chat history.
- The task pane requires HTTPS, so local development proxies `/api` to Accelerate Legal
...
2. Copy and edit the environment file:
   ...
   REACT_APP_WEB_APP_URL=https://app.accelerateai.io
...
4. Start the Accelerate Legal backend:
...
The task pane appears under **Home → Accelerate Legal → Accelerate Legal**. `npm run
...
Restart Word, then choose **Insert → Add-ins → My Add-ins → Accelerate Legal**.
...
The add-in host must reverse-proxy `https://word.example.com/api/*` to the Accelerate Legal
...
still scoped to the signed-in Accelerate Legal account. A same-account **Save As** copy can
...
does not require Word, Supabase, or a live Accelerate Legal API:
...
Confirm the Accelerate Legal API is reachable at the configured URL, the object-storage
