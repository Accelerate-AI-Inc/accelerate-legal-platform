...
couple of the existing suites first (`src/app/lib/accelerateApi.test.ts`,
...
| `lib/utils.ts` | 100 | ✓ |
| `lib/supabase.ts` | 100 | ✓ |
| `lib/accelerateApi.ts` | 99.77 | ✓ — every endpoint wrapper asserted |
...
couple of `?? null` default arms. `accelerateApi.ts` now has a route/method/body
...
- [x] `lib/accelerateApi.ts` (rest) — the remaining thin wrappers: folders/library
...
- [ ] `hooks/useFetchSingleDoc.ts` + `useFetchDocxBytes.ts` — fetch/refresh
      lifecycle with mocked `accelerateApi`. (S)
