...
`e2e/auth.setup.ts` bootstraps the shared test user (`e2e@accelerate.local`) against
...
and fork PRs with no secret access) green and fast. Accelerate Legal supports keyless local
...
```bash
gh secret set ANTHROPIC_API_KEY --repo accelerate-ai/accelerate-legal
