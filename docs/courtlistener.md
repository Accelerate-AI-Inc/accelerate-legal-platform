...
Accelerate Legal can use CourtListener for US case-law citation verification, case fetching,
...
Set `COURTLISTENER_BULK_DATA_ENABLED=true` to make Accelerate Legal try local Supabase and
...
Accelerate Legal falls back to the live API when local bulk data is unavailable and a token
is configured.
