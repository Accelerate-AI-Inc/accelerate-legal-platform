...
1. `frontend/src/app/components/ui/` contains reusable web primitives such as
   buttons, inputs, dropdowns, search, empty states, and the composed surface
   classes.
...
Design changes should follow the existing editorial theme — opaque surfaces,
hairline rules, no bespoke shadows — and use Lucide icons. Prefer the tokens and shared class constants
in `frontend/src/shared/ui/TokensUI.css` and
`frontend/src/app/components/ui/surface.ts` over raw palette values,
...
- Calls to the Express backend belong in `frontend/src/app/lib/accelerateApi.ts` so
...
`backend/src/lib/httpError.ts`; logging must not include raw provider or
database payloads. Intentional validation and permission failures
