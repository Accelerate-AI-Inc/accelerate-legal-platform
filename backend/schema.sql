-- Accelerate Legal Supabase schema
...
-- newer than the version of Accelerate Legal they currently have deployed.
...
create table if not exists public.accelerate_workflows (
  id uuid primary key default gen_random_uuid(),
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint accelerate_workflows_key_hash_unique
    unique(workflow_key, content_hash),
  constraint accelerate_workflows_distribution_check
    check(distribution in ('default', 'addon')),
  constraint accelerate_workflows_type_check
    check(type in ('assistant', 'tabular')),
  constraint accelerate_workflows_source_commit_check
    check(source_commit is null or source_commit ~ '^[0-9a-f]{40}$'),
  constraint accelerate_workflows_content_hash_check
    check(content_hash ~ '^[0-9a-f]{64}$')
);

create unique index if not exists accelerate_workflows_active_key_idx
  on public.accelerate_workflows(workflow_key)
  where active;

create index if not exists accelerate_workflows_active_distribution_type_idx
  on public.accelerate_workflows(active, distribution, type, title);

create index if not exists accelerate_workflows_active_pack_idx
  on public.accelerate_workflows(active, pack_key, title);
...
create table if not exists public.accelerate_workflow_reference_files (
  id uuid primary key default gen_random_uuid(),
  accelerate_workflow_id uuid not null
    references public.accelerate_workflows(id) on delete cascade,
  filename text not null,
  file_type text not null,
  storage_path text not null,
  size_bytes integer,
  content_hash text not null,
  created_at timestamptz not null default now(),
  constraint accelerate_workflow_reference_files_name_unique
    unique(accelerate_workflow_id, filename),
  constraint accelerate_workflow_reference_files_hash_check
    check(content_hash ~ '^[0-9a-f]{64}$')
);
...
-- accelerate_workflows. New code calls the one-argument function above.
create or replace function public.install_missing_default_workflows(
  p_user_id text,
  p_defaults jsonb
)
...
--
-- Curated reference data with no per-user ownership, seeded from
-- backend/src/lib/resources/legalAiResources.json by `npm run sync:resources`.
-- Same shape as the workflow catalog: content-addressed rows replaced in one
-- transaction, with a partial unique index over the active set.
-- ---------------------------------------------------------------------------
...
-- accelerate_workflows table and have no user-data growth. They are deliberately
