...
import { LEGAL_RESOURCE_TOOLS } from "./tools/legalResourceTools";
import {
...
export async function runLLMStream(params: {
  ...
  // The resources catalog is a research capability, so it follows the same
  // gate as CourtListener: available in the web assistant, withheld from
  // document-scoped Word chats.
  const researchTools = includeResearchTools
    ? [...COURTLISTENER_TOOLS, ...LEGAL_RESOURCE_TOOLS]
    : [];
