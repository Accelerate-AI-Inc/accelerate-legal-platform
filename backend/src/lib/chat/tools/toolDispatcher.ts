...
export async function runToolCalls(
  ...
  legalResourceEvents: LegalResourceToolEvent[];
  ...
  const legalResourceEvents: LegalResourceToolEvent[] = [];
  ...
  for (const tc of toolCalls) {
    ...
    } else if (tc.function.name === LEGAL_RESOURCE_TOOL_NAME) {
      write(
        `data: ${JSON.stringify({ type: "legal_resource_lookup_start", query: typeof args.query === "string" ? args.query : null })}\n\n`,
      );
      const lookup = await runLegalResourceLookup(db, args);
      legalResourceEvents.push(lookup.event);
      write(`data: ${JSON.stringify(lookup.event)}\n\n`);
      toolResults.push({
        role: "tool",
        tool_call_id: tc.id,
