...
/**
 * Workflow front-matter keys are a contract with a separate content
 * repository, which cannot be renamed in the same commit as this one. Reads
 * accept the current namespace and fall back to the pre-rename one; new
 * content should use the current namespace.
 */
const METADATA_PREFIX = "accelerate";
const LEGACY_METADATA_PREFIX = "mike";

function metadataValue(
  metadata: Record<string, unknown>,
  key: string,
): unknown {
  const current = metadata[`${METADATA_PREFIX}-${key}`];
  return current !== undefined
    ? current
    : metadata[`${LEGACY_METADATA_PREFIX}-${key}`];
}

const DEFAULT_REPOSITORY = "accelerate-ai/accelerate-workflows";
...
function githubHeaders(token?: string): Record<string, string> {
  return {
    ...
    "User-Agent": "accelerate-workflow-sync",
...
function sourceConfiguration(options: WorkflowCatalogSourceOptions) {
  const repository =
    options.repository ??
    process.env.ACCELERATE_WORKFLOWS_REPOSITORY ??
    DEFAULT_REPOSITORY;
  const ref = options.ref ?? process.env.ACCELERATE_WORKFLOWS_REF ?? DEFAULT_REF;
  const githubToken =
    options.githubToken ?? process.env.ACCELERATE_WORKFLOWS_GITHUB_TOKEN;
  if (!REPOSITORY_PATTERN.test(repository)) {
    throw new Error(
      "ACCELERATE_WORKFLOWS_REPOSITORY must use the owner/repository form",
    );
  }
  if (!ref.trim() || ref.length > 200) {
    throw new Error("ACCELERATE_WORKFLOWS_REF must be between 1 and 200 characters");
  }
...
async function downloadArchive(
  ...
  if (declaredLength > MAX_ARCHIVE_BYTES) {
    throw new Error(
      "Accelerate Legal workflows archive exceeds the 10 MB compressed limit",
    );
  }
  if (!response.body) {
    throw new Error("GitHub returned an empty Accelerate Legal workflows archive");
  }
  ...
  try {
    while (true) {
      ...
      while (offset < value.byteLength) {
        ...
        if (bytesWritten === 0) {
          throw new Error("Could not write the Accelerate Legal workflows archive");
        }
        offset += bytesWritten;
      }
    }
...
async function parseArchive(
  ...
  for (const skillPath of skillPaths) {
    ...
    const metadataType = requiredString(
      ...
      `${skillPath}.metadata.${METADATA_PREFIX}-type`,
    );
    if (metadataType !== type) {
      throw new Error(
        `${skillPath}.metadata.${METADATA_PREFIX}-type must be '${type}'`,
      );
    }
    const availability = optionalString(
      metadataValue(metadata, "availability"),
      `${skillPath}.metadata.${METADATA_PREFIX}-availability`,
    );
    if (
      availability &&
      availability !== "system" &&
      ...
      throw new Error(
        `${skillPath}.metadata.${METADATA_PREFIX}-availability is invalid`,
      );
    }

    const title = requiredString(
      metadataValue(metadata, "display-name"),
      `${skillPath}.metadata.${METADATA_PREFIX}-display-name`,
    );
...
export async function prepareWorkflowCatalog(
  ...
  const directory = await mkdtemp(
    path.join(options.temporaryRoot ?? tmpdir(), "accelerate-workflows-"),
  );
  try {
