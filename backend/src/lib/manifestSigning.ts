 ...
 * edited one.
 *
 * Optional by design. Accelerate Legal self-hosts, and a deployment that has not set up a
 ...
 */
export const SIGNING_CONTEXT = "accelerate-project-manifest-v1";

/**
 * The context string before the product rename. Nothing here signs with it any
 * more, but manifests exported before the rename carry signatures over it, so
 * any verifier must accept both. Exported so external verification tooling has
 * one place to read the accepted set from.
 */
export const LEGACY_SIGNING_CONTEXTS = ["mike-project-manifest-v1"] as const;
