...
/**
 * As with the BYOK salt: the salt is part of the stored ciphertext's identity,
 * so writes use the current one and reads fall back to the pre-rename salt.
 * Keep LEGACY_MCP_SALT until no deployment holds pre-rename connector rows.
 */
const MCP_SALT = "accelerate-user-mcp-v1";
const LEGACY_MCP_SALT = "mike-user-mcp-v1";

function encryptionKey(salt: string = MCP_SALT): Buffer {
...
/** Decrypt with the current salt, falling back to the pre-rename salt. */
function decryptRaw(encrypted: string, iv: string, tag: string): string {
    let lastError: unknown;
    for (const salt of [MCP_SALT, LEGACY_MCP_SALT]) {
        try {
...
export function decryptString(
    ...
    try {
        return decryptRaw(encrypted, iv, tag);
    } catch (err) {
