...
/**
 * scrypt salts are part of the stored ciphertext's identity: change one and
 * every key already in the database becomes undecryptable. The salt was
 * renamed with the product, so writes use the current salt and reads fall
 * back to the legacy one, re-encrypting in place on the way through. Keep
 * LEGACY_KEY_SALT until no deployment holds pre-rename rows.
 */
const KEY_SALT = "accelerate-user-api-keys-v1";
const LEGACY_KEY_SALT = "mike-user-api-keys-v1";
...
function encryptionKey(salt: string = KEY_SALT): Buffer {
...
function decryptWithSalt(row: EncryptedKeyRow, salt: string): string {
...
function decrypt(row: EncryptedKeyRow): string | null {
    try {
        return decryptWithSalt(row, KEY_SALT);
    } catch {
        // Fall through to the pre-rename salt before treating the row as lost.
    }
    try {
        return decryptWithSalt(row, LEGACY_KEY_SALT);
    } catch (err) {
