...
/**
 * Read-only browsing of the curated resources catalog. Search-as-you-type on a
 * small reference table is cheap but chatty, so it gets its own generous lane
 * rather than spending the general 300/15m budget the rest of the app shares.
 */
const catalogBrowseLimiter = makeLimiter({
  windowMs: minutes(envInt("RATE_LIMIT_CATALOG_WINDOW_MINUTES", 15)),
  max: envInt("RATE_LIMIT_CATALOG_MAX", 600),
...
app.get("/legal-resources", catalogBrowseLimiter);
app.get("/legal-resources/filter-options", catalogBrowseLimiter);
app.get("/legal-resources/:slug", catalogBrowseLimiter);
