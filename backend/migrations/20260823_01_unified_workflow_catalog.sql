...
-- Store every Accelerate Legal-authored workflow in one versioned runtime catalog.
...
begin
  ...
  if to_regclass('public.workflow_addon_reference_files') is not null
    ...
    execute $references$
      ...
      join public.accelerate_workflows catalog
        on catalog.workflow_key = addon.addon_key
       and catalog.content_hash = addon.content_hash
      on conflict (accelerate_workflow_id, filename) do nothing
    $references$;
  end if;
end;
$$;
