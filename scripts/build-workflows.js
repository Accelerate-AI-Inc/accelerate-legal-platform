...
const WORKFLOWS_DIR = process.env.ACCELERATE_WORKFLOWS_DIR
  ? path.resolve(process.env.ACCELERATE_WORKFLOWS_DIR)
  : path.join(WORKSPACE_DIR, "accelerate-workflows");
...
/**
 * Workflow front-matter keys are a contract with the separate content
 * repository, which cannot be renamed in the same commit as this one. Reads
 * accept the current namespace and fall back to the pre-rename one. Mirrors
 * `metadataValue` in backend/src/lib/workflowCatalogSource.ts.
 */
...
// The commit of the accelerate-workflows checkout the generated files were built
...
function readWorkflow(category, workflowDir, pack) {
  ...
  assertString(
    ...
    `${label}.metadata.${METADATA_PREFIX}-display-name`,
  );
  if (metadataValue(metadata, "type") !== category) {
    fail(`${label}.metadata.${METADATA_PREFIX}-type must be "${category}"`);
  }
  // metadata.<prefix>-availability is deprecated: backend ingestion derives the
  ...
  const availability = metadataValue(metadata, "availability");
  if (
    availability !== undefined &&
    !["system", "add-on"].includes(availability)
  ) {
    console.warn(
      `Warning: ${label}.metadata.${METADATA_PREFIX}-availability has unexpected value ${JSON.stringify(availability)}; the key is deprecated and ignored`,
    );
  }
  ...
  const normalizedMetadata = {
    ...
    title: metadataValue(metadata, "display-name"),
...
function main() {
  ...
  console.log(
    `${generated ? "Generated" : "Validated"} ${workflows.length} landing workflows from accelerate-workflows @ ${sourceCommit}.`,
  );
}
