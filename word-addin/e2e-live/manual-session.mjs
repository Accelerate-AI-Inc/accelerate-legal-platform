 ...
 * creates a new document, sideloads the Accelerate Legal manifest, opens the task
 ...
 * the Accelerate Legal backend on :3001 (the same services used by scripts/dev.sh).
 */
...
const PROFILE = path.join(os.homedir(), ".cache", "accelerate-word-web-profile");
...
console.log("Sideloading the Accelerate Legal manifest…");
...
console.log("Waiting for the Accelerate Legal pane…");
try {
  await waitForFrame((f) => f.url().includes("localhost:3200"), 30_000);
} catch {
  await wac.getByRole("button", { name: /accelerate/i }).first().click();
  await waitForFrame((f) => f.url().includes("localhost:3200"), 60_000);
}

console.log(
  "READY — Word + Accelerate Legal pane are yours. Test manually; close the browser window to end.",
);
