...
test("uses a floating icon header with no logo, tabs, or visible sign-out button", async ({
  ...
  await expect(page.getByText("Accelerate Legal", { exact: true })).toHaveCount(0);
...
test("does not send without the required Word document context", async ({
  ...
  await expect(page.getByRole("alert")).toHaveText(
    "Accelerate Legal couldn't read the current Word document. Please try again.",
  );
...
test("uses Web Crypto for document IDs when randomUUID is unavailable", async ({
  ...
  expect((await addin.wordDocument()).settings["accelerate.word.documentId.v1"]).toBe(
    body.document_id,
  );
});
...
test("a Save As copy of the document mints a fresh chat identity", async ({
  ...
  await addin.setWordDocumentSetting("accelerate.wordEditAnchors.v1", {
    version: 1,
    anchors: {},
  });
  await addin.gotoTaskpane({
  ...
  expect(settings["accelerate.word.documentId.v1"]).toBe(secondId);
  expect(settings["accelerate.word.documentUrl.v1"]).toBe(
    "c:/users/e2e/demo contract (copy).docx",
  );
  expect(settings["accelerate.wordEditAnchors.v1"]).toBeUndefined();
});
...
test("keeps the existing identity when either document URL is unknown", async ({
  ...
  await addin.removeWordDocumentSetting("accelerate.word.documentUrl.v1");
  await addin.gotoTaskpane({
  ...
  expect(
    (await addin.wordDocument()).settings["accelerate.word.documentUrl.v1"],
...
test("the composer swaps Send for a Stop control while streaming, then restores", async ({
  ...
  const accelerateLoader = responseStatus.locator('svg[viewBox="100 100 300 300"]');
  ...
  await expect(accelerateLoader).toBeVisible();
  await expect(
    page
      .locator("[data-assistant-message-id]")
      .last()
...
test("does not broaden one edit across repeated exact passages", async ({
  ...
  await expect(
    page.getByText(
      "Skipped — this text appears 2 times in the document. Tell Accelerate Legal which one to change.",
    ),
  ).toBeVisible();
  await expect(
    page.getByRole("button", { name: "Accept", exact: true }),
