...
const ANCHOR_SETTINGS_KEY = "accelerate.wordEditAnchors.v1";
...
test("rehydrates View from a hidden Word bookmark after the task pane reloads", async ({
  ...
  expect(bookmark.name).toMatch(/^_AxEdit_[A-Za-z0-9_]+$/);
...
test("keeps review controls when the bookmark holds an unrelated sibling revision", async ({
  ...
  // Word ranges report sibling revisions from the same passage; Accelerate Legal's own
  ...
  // Accepting resolves exactly Accelerate Legal's pair — never the injected revision.
  await accept.click();
  await expect(page.getByText("Accepted.", { exact: true })).toBeVisible();
  const calls = await addin.wordCalls();
  expect(calls.acceptedChanges).toEqual([
...
test("stays View-only when the bookmark's revisions no longer identify the edit", async ({
  ...
  // A second identical Added revision makes Accelerate Legal's insertion ambiguous, so
