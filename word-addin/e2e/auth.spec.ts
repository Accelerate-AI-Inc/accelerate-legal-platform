/**
 * Auth flow E2E coverage for the Accelerate Legal Word add-in.
 *
 ...
 *
 * The Accelerate Legal backend auth endpoints are mocked via addin.mockLogin and the shared
...
test.describe("auth flow", () => {
  test("resolves the loading spinner into the login page when no token is stored", async ({
    ...
    await expect(page.getByText("Accelerate Legal", { exact: true })).toBeVisible();
