...
module.exports = async (_env, options) => {
  ...
  if (isDev) {
    ...
    // (HTTP requests from an HTTPS page). Proxy the local Accelerate Legal API through this
  ...
  const config = {
    ...
    resolve: {
      ...
      alias: {
        ...
        "@ax/edit-card-ui": frontendSharedUi("EditCardUI.tsx"),
        "@ax/edit-cards-section-ui": frontendSharedUi(
          "EditCardsSectionUI.tsx",
        ),
        "@ax/pre-response-wrapper-ui": frontendSharedUi(
          "PreResponseWrapperUI.tsx",
        ),
        "@ax/document-event-blocks-ui": frontendSharedUi(
          "DocumentEventBlocksUI.tsx",
        ),
        "@ax/card-ui": frontendSharedUi("CardUI.tsx"),
        "@ax/modal-ui": frontendSharedUi("ModalUI.tsx"),
        "@ax/header-buttons-ui": frontendSharedUi(
          "HeaderButtonsUI.tsx",
        ),
        "@ax/pill-button-ui": frontendSharedUi("PillButtonUI.tsx"),
        "@ax/dropdown-ui": frontendSharedUi("DropdownUI.tsx"),
        "@ax/citation-pill-ui": frontendSharedUi("CitationPillUI.tsx"),
        "@ax/model-toggle-ui": frontendSharedUi("ModelToggleUI.tsx"),
        "@ax/brand-mark-ui": frontendSharedUi("BrandMarkUI.tsx"),
        "@ax/google-icon-ui": frontendSharedUi("GoogleIconUI.tsx"),
        "@ax/auth-styles-ui": frontendSharedUi("AuthStylesUI.ts"),
        "@ax/auth-divider-ui": frontendSharedUi("AuthDividerUI.tsx"),
        "@ax/workflow-slash-command-ui": frontendSharedUi(
    ...
    plugins: [
      ...
      new webpack.EnvironmentPlugin({
        ...
        // Accelerate Legal backend so HttpOnly auth cookies are never third-party cookies.
        REACT_APP_API_BASE_URL:
          process.env.REACT_APP_API_BASE_URL || "/api",
        REACT_APP_DEFAULT_MODEL: "gemini-3-flash-preview",
        // The Accelerate Legal web app origin — the task pane links here (e.g. the
        // account/api-keys page); it never fetches from it.
        REACT_APP_WEB_APP_URL: isDev
          ? process.env.REACT_APP_WEB_APP_URL || "https://app.accelerateai.io"
          : undefined,
      }),
    ],
