...
import { CitationPillUI } from "@ax/citation-pill-ui";
...
const MARKDOWN_COMPONENTS: Components = {
    ...
    if (href?.startsWith("#accelerate-cite:")) {
      ...
      return (
        <a
          ...
          data-accelerate-citation=""
          title="Show in the document"
          className="mx-0.5 inline-flex max-w-full cursor-pointer items-baseline rounded-sm border border-rule bg-paper px-1.5 py-0.5 align-baseline font-sans text-[0.85em] not-italic text-ink-muted no-underline transition-colors hover:border-rule-strong hover:bg-surface-sunk hover:text-ink"
        >
  ...
  blockquote: ({ children }) => (
    <blockquote className="mb-3 border-l-2 border-rule-strong pl-3 text-ink-muted italic last:mb-0">
      {children}
    </blockquote>
  ),
  code: ({ className: codeClass, children }) => {
    ...
    if (isBlock) {
      return (
        <code className="block overflow-x-auto rounded-sm bg-surface-sunk p-3 font-mono text-xs text-ink">
          {children}
        </code>
      );
    }
    return (
      <code className="rounded bg-surface-sunk px-1 py-0.5 font-mono text-[0.85em] text-ink">
        {children}
      </code>
    );
  },
  ...
  th: ({ children }) => (
    <th className="border border-rule bg-paper px-2 py-1 text-left font-semibold">
      {children}
    </th>
  ),
  td: ({ children }) => (
