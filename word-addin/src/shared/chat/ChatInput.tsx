 ...
 * the same veil container and solid-ink square action button,
...
export function ChatInput({
    ...
    return (
        <div
            ...
            className={cn(
                "rounded-[18px] border border-rule/65 bg-surface/60",
                className,
            )}
        >
            {attachments && (
            ...
            <div ref={inputAreaRef} className="px-4 pt-4">
                <textarea
                    ...
                    disabled={disabled}
                    className="w-full resize-none overflow-hidden border-0 bg-transparent p-0 text-sm leading-6 text-ink outline-none placeholder:text-ink-faint disabled:opacity-60"
                />
            </div>
            <div className="flex items-center justify-between gap-2 p-2">
                ...
                <div className="flex shrink-0 items-center gap-1">
                    ...
                    <button
                        ...
                        className={cn(
                            "relative flex h-8 w-8 shrink-0 cursor-pointer items-center justify-center rounded-sm border-0 bg-ink text-paper transition-all duration-150 active:enabled:scale-95 disabled:cursor-default disabled:bg-ink-faint",
                            "",
                        )}
                    >
                        {isLoading ? (
