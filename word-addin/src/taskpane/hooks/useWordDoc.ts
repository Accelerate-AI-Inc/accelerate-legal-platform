 ...
 * Accelerate Legal edit. Callers must treat the string as opaque.
 */
...
function createTrackedEditHandle(): TrackedEditHandle {
  ...
  return `accelerate-edit-${Date.now().toString(36)}-${nextTrackedEditHandle.toString(36)}-${createSecureUuid()}` as TrackedEditHandle;
}
 ...
 * document-wide rule, and it is what lets two pending Accelerate Legal edits that share
...
async function resolveTrackedEditNow(
  ...
  try {
    ...
    if (!resolvedViaChildren) {
      // Resolve through the edited passage: it was revision-free before Accelerate Legal
      // touched it and every mutation below re-verifies content first, which
      // keeps the scope of this decision to Accelerate Legal's own edit. Word does not
    ...
    if (error instanceof ChangedRevisionSetError) {
      ...
      return {
        ...
        status: "error",
        error:
          "The revisions in this passage changed after Accelerate Legal applied the edit. Review them directly in Word.",
      };
    }
 ...
 * the bookmarked passage from being accepted or rejected by Accelerate Legal.
 */
...
export function useWordDoc() {
   ...
   *
   * This powers the conflicted card's "Accept & apply": Accelerate Legal never layers a
          ...
          try {
            ...
            for (const [index, suppliedEdit] of edits.entries()) {
              ...
              try {
                ...
                // Never mix Accelerate Legal's lifecycle with revisions that were already
                ...
                // subset is still Accelerate Legal's edit (the target was revision-free a
                ...
                result.error = mutationApplied
                  ? "Applied in Word, but Accelerate Legal couldn’t retain its review controls. Review it from Word’s Review tab."
                  : describeWordFailure(
                      error,
