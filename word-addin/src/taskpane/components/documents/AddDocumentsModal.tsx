  ...
  return (
    <div ref={rowRef} className="flex justify-center py-2">
      <button
        ...
        disabled={loading}
        onClick={onLoadMore}
        className="flex items-center gap-1.5 rounded-full px-3 py-1.5 text-[11px] text-ink-muted hover:text-ink disabled:opacity-60"
      >
  ...
  const renderDocument = (document: Document, depth = 0) => {
    ...
    return (
      <button
        ...
        className={`w-full min-w-0 rounded-sm py-2 pr-2 text-left text-xs transition-all ${DIRECTORY_GRID_CLASS} ${
          selected
      ...
      >
        ...
        >
          {selected && <Check className="h-2.5 w-2.5 text-paper" />}
        </span>
      ...
      return (
        <div key={folder.id}>
          <button
            ...
            style={{ paddingLeft: 8 + depth * 16 }}
            className={`w-full rounded-sm py-2 pr-2 text-left text-xs text-ink transition-all hover:bg-surface-sunk ${DIRECTORY_GRID_CLASS}`}
          >
            <span
              role="checkbox"
            ...
            >
              {allSelected && <Check className="h-2.5 w-2.5 text-paper" />}
              {someSelected && <span className="h-px w-2 bg-surface" />}
            </span>
            {folderLoading ? (
  ...
  return (
    ...
    >
      ...
      {(warning || error) && (
        <div
          role="alert"
          className="mb-2 flex items-center gap-2 rounded-sm border border-critical bg-critical-wash px-3 py-2 text-xs text-ink"
        >
          ...
          <button
            ...
            className="shrink-0 rounded p-0.5 text-ink-muted hover:bg-surface-sunk"
                  ...
                  return (
                    <div key={project.id}>
                      <button
                        ...
                        className={`w-full rounded-sm px-2 py-2.5 text-left text-xs text-ink transition-all hover:bg-surface-sunk ${DIRECTORY_GRID_CLASS}`}
                      >
                        <span
                          role="checkbox"
                          aria-checked={someSelected ? "mixed" : allSelected}
                        ...
                        >
                          {projectLoading ? (
                            <Loader2 className="h-2.5 w-2.5 animate-spin text-ink-faint" />
                          ) : allSelected ? (
                            <Check className="h-2.5 w-2.5 text-paper" />
                          ) : someSelected ? (
                            <span className="h-px w-2 bg-surface" />
                          ) : null}
                        </span>
                        <ProjectSvgIcon
                          open={expanded}
                          ...
                          <div>
                            ...
                            {projectLoaded &&
                              ...
                              childFolders(folders, null).length === 0 && (
                                <p className="py-2 pl-9 text-xs text-ink-faint">
                                  No documents
                                </p>
                              )}
                          </div>
            ...
            <>
              {activeTab === "files" &&
                uploadingFilenames.map((filename) => (
                  <div
                    key={filename}
                    className={`px-2 py-2 text-xs text-ink-faint ${DIRECTORY_GRID_CLASS}`}
                  >
                    <span className="h-3.5 w-3.5 rounded border border-rule-strong" />
                    <Loader2 className="h-3.5 w-3.5 animate-spin" />
