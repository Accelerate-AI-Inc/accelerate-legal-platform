      ...
      <div className="max-w-[80%] bg-surface-sunk rounded-sm px-4 py-3">
        ...
        >
          ...
          {canExpand && !expanded && (
            <div
              aria-hidden="true"
              className="pointer-events-none absolute inset-x-0 bottom-0 h-16 bg-gradient-to-b from-transparent via-surface-sunk/80 to-surface-sunk backdrop-blur-[2px] [mask-image:linear-gradient(to_bottom,transparent,black_55%)]"
            />
          )}
          {canExpand && (
            <button
              ...
              className="absolute bottom-0 left-1/2 z-10 inline-flex h-7 w-9 -translate-x-1/2 items-center justify-center rounded-sm text-ink-muted transition-colors hover:bg-surface-sunk/90 hover:text-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/40"
            >
              {expanded ? (
        ...
        {(workflow || (files && files.length > 0)) && (
          <div className="mt-3 flex flex-wrap justify-end gap-1.5">
            {workflow && (
              <div className="inline-flex items-center gap-1 rounded-full border border-accent bg-accent py-0.5 pl-2 pr-2.5 text-xs text-paper shadow">
            ...
            {files?.map((file) => (
              <div
                ...
                className="inline-flex items-center gap-1 rounded-[10px] border border-rule/70 bg-surface py-0.5 pl-2 pr-2.5 text-xs text-ink"
              >
                <FileText className="h-2.5 w-2.5 shrink-0 text-ink-faint" />
