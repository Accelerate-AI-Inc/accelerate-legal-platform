...
function AssistantMessageImpl({
  ...
  const handleCitationClick = React.useCallback(
    (event: React.MouseEvent<HTMLDivElement>) => {
      ...
      const citationControl = target.closest?.("[data-accelerate-citation]");
      const href =
    ...
    >
      ...
      <div className="mt-2 flex flex-col gap-3">
            ...
            return (
              <React.Fragment key={group.key}>
                {group.text && !holdForEdit && (
                  <div
                    className="font-serif text-base leading-7 text-ink"
        ...
        {error && (
          <p
            role="alert"
            className="font-serif text-base leading-7 text-critical"
          >
            {error}
          </p>
        )}
