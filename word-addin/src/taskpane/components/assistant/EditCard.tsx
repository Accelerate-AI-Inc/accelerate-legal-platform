...
import { EditCardUI } from "@ax/edit-card-ui";
...
> = {
  receiving: { copy: "Receiving change…", className: "text-ink-faint" },
  validating: { copy: "Checking the document…", className: "text-ink-faint" },
  applying: { copy: "Applying to the document…", className: "text-ink-muted" },
  restoring: { copy: "Checking the document…", className: "text-ink-faint" },
  "view-only": {
    copy: "Tracked change found — review it in Word.",
    className: "text-ink-muted",
  },
  applied: { copy: "Applied to the document.", className: "text-positive" },
  accepted: { copy: "Accepted.", className: "text-positive" },
  rejected: { copy: "Rejected.", className: "text-ink-muted" },
  skipped: {
  ...
  error: { copy: "Couldn’t apply this change.", className: "text-critical" },
  historical: { copy: "Historical change.", className: "text-ink-faint" },
};
...
export function EditCard({
  ...
  const ambiguousCopy =
    status === "ambiguous" && matches !== undefined && matches > 1
      ? `Skipped — this text appears ${matches} times in the document. Tell Accelerate Legal which one to change.`
      : undefined;
    ...
    <EditCardUI
      ...
      previewContent={
        ...
        isFormatCard && hasEditText ? (
          <>
            <span className={`text-ink ${formatPreviewClass}`}>
              {edit.original}
            </span>
            <span className="ml-2 text-ink-faint">{formatLabels}</span>
          </>
        ) : undefined
      }
      reason={edit.reason}
