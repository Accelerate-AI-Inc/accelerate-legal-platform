    ...
    <Dropdown open={open} onOpenChange={setOpen}>
      <DropdownTrigger asChild>
        <button
          ...
          className={`flex h-8 shrink-0 items-center gap-1 rounded-full px-1.5 text-sm text-ink-faint transition-colors hover:text-ink ${
            open ? "text-ink" : ""
          }`}
        >
        ...
        {OPTIONS.map((option) => (
          ...
          >
            <span className="flex items-center gap-2">
              <option.Icon className="h-3.5 w-3.5 shrink-0 text-ink-muted" />
            ...
            <span className="pl-[22px] text-xs text-ink-muted">
              {option.description}
            </span>
          </DropdownItem>
        ))}
