...
function ToolbarButton({
  ...
  return (
    <button
      ...
      className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-sm text-ink-muted transition-colors hover:bg-surface hover:text-ink ${
        active ? "bg-surface-sunk text-ink hover:bg-surface-sunk" : ""
      }`}
    >
      {children}
    </button>
...
export function WorkflowPromptEditor({
  ...
  return (
    <div className="flex h-full min-h-[240px] flex-col overflow-hidden rounded-sm border border-rule/70 bg-surface/55">
      <div className="flex min-h-10 shrink-0 items-center gap-0.5 border-b border-rule/70 bg-surface/45 px-2 py-1.5">
        {readOnly ? (
          <span className="mr-auto px-2 text-xs font-medium text-ink-muted">
            Read-only
          </span>
        ) : (
          <>
            ...
            <span className="mx-1 h-4 w-px shrink-0 bg-surface-sunk" />
            <ToolbarButton
              title="Bold"
            ...
            <div ref={tablePickerRef} className="relative">
              ...
              {tablePickerOpen && (
                <div
                  ...
                  className="absolute left-0 top-full z-[250] mt-1 rounded-sm border border-rule bg-surface p-2"
                >
                  <div
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        {rawMode ? (
          <textarea
            ...
            className="h-full min-h-full w-full resize-none bg-transparent px-4 py-3 font-mono text-xs leading-6 text-ink outline-none read-only:cursor-default"
          />
        ) : (
