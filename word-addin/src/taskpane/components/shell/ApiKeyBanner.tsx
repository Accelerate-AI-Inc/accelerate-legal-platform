...
import { getApiKeyStatus, type ApiKeyStatus } from "../../api/accelerateApi";
...
export function ApiKeyBanner(): React.ReactElement | null {
  ...
  return (
    <div
      role="status"
      className="shrink-0 border-b border-caution bg-caution-wash px-3 py-2 text-xs text-caution"
    >
      <div className="flex items-start gap-2">
        <KeyRound className="mt-0.5 h-3.5 w-3.5 shrink-0 text-caution" />
        ...
        <button
          ...
          aria-label="Dismiss"
          className="shrink-0 rounded p-0.5 text-caution transition-colors hover:bg-caution-wash hover:text-caution"
        >
      ...
      <button
        ...
        className="mt-1.5 ml-[22px] rounded-sm bg-caution px-2 py-1 text-[11px] font-medium text-paper transition-colors hover:bg-caution"
      >
        Set up API keys
      </button>
    </div>
