...
before(async () => {
  staticRoot = await mkdtemp(path.join(os.tmpdir(), "accelerate-word-host-"));
  await writeFile(
    path.join(staticRoot, "taskpane.html"),
    "<!doctype html><title>Accelerate Legal Word</title>",
  );
...
test("serves health and the task pane", async () => {
  ...
  assert.match(await taskpane.text(), /Accelerate Legal Word/);
});
