# Accelerate Legal Word Add-in

The Accelerate Legal Word add-in brings document-aware chat, workflows, quick actions, and
tracked-edit review into a Word task pane.

It uses the same backend-managed HttpOnly cookie session, Accelerate Legal API, model
...
- A running Accelerate Legal backend and Supabase environment configured according to the
  [local development guide](../docs/local-development.md)
- A Accelerate Legal account
...
HTTPS certificate, verifies Accelerate Legal, and launches the add-in in Word
...
- Attach Accelerate Legal library documents and assistant workflows
...
- Store chat history in Accelerate Legal Cloud or on the current device only
...
- **Word desktop on macOS:** copy `manifest.xml` into
  ...
  then open **Insert → Add-ins → My Add-ins → Accelerate Legal**.
